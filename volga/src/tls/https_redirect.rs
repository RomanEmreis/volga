...
use crate::{HttpBody, error::Error, status};
use futures_util::future::BoxFuture;

use hyper::{
    Request, Response, Uri, body::Incoming, header::HOST, http::uri::Scheme, service::Service,
};
    ...
    fn call(&self, request: Request<Incoming>) -> Self::Future {
        ...
        Box::pin(async move {
            ...
            if let Some(host) = parts.headers.get(&HOST).and_then(|host| host.to_str().ok()) {
                ...
                let uri =
                    Uri::from_parts(uri_parts).map_err(HttpsRedirectionError::invalid_uri_parts)?;
