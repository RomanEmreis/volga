...
mod tests {
    ...
    fn it_extracts_option_future_poll_ready_ok() {
        ...
        let mut option_future = OptionFromPayloadFuture {
            inner: inner_future,
        };
    ...
    async fn it_extracts_option_integration_with_real_extractors() {
        ...
        let result =
            Option::<NamedPath<Params>>::from_payload(Payload::PathArgs(&PathArgs::new())).await;
        assert!(result.is_ok());
        assert!(result.unwrap().is_none());
    }
    ...
    async fn it_extracts_option_integration_with_path_extractor() {
        ...
        let result =
            Option::<Path<(i32, String)>>::from_payload(Payload::PathArgs(&PathArgs::new())).await;
        assert!(result.is_ok());
        assert!(result.unwrap().is_none());
    }
}
