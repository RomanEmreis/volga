...
* Failures are reported under the name the client sent, not the Rust field name: `#[serde(rename = "..")]` and `#[serde(rename_all = "..")]` are read off the type, and `#[validate(rename = "..")]` overrides both. A `#[serde(flatten)]` field is reported at the level the client sent it, without the name it carries in Rust. `nested` merges a nested type's failures under `parent.child`, and a nested collection under `parent[0].child`. `ValidationError::merge` and `merge_at` do that merging and are usable by hand.
