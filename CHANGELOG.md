...
## Added
* `#[derive(Validate)]` (feature `validation-derive`, #218) - writes out the same `validate()` a hand-written impl would, from the bounds declared on the fields. It is not magic and it is not a second way to validate: the trait stays the seam, so a derived impl and a hand-written one are indistinguishable to `Valid<E>`. Field rules are `length(min, max, equal)` for strings and collections, `range(min, max)` for anything ordered, `nested` for a field validating itself, and `custom = "path::to::fn"`; each takes an optional `message = ".."` that replaces the default text. An attribute the derive could only half-honour is refused rather than half-applied: `equal` alongside `min` or `max`, since `equal` decides the check on its own, and a `range` bound that is neither a literal nor a constant, since a bound is read once when the route is described and again on every request - anything that could answer differently between the two would publish a contract the server does not enforce. A rule that genuinely varies belongs in `custom`. A rule spanning two fields cannot live on either of them, so the container takes `schema = "path::to::fn"` for those - without it, cross-field rules would have nowhere to go, which is the reason the derive is sugar over the trait rather than a replacement for it. All failures are collected rather than stopping at the first, and every bound and message is rendered at expansion time, so a failing check pushes a `&'static str` and allocates nothing. A status asked for by a merged failure is kept, so `ValidationError::with_status` works from a `schema` or `custom` function exactly as it does from a hand-written impl.
* The derived rules are published as well as enforced: `Validate::constraints()` reports what the fields declare, `Valid<E>` forwards it into the OpenAPI description, and it lands on the property of the request schema or on the query parameter of the same name, under the keyword the field's shape calls for: `minLength` / `maxLength` for a string, `minItems` / `maxItems` for a collection, `minProperties` / `maxProperties` for a map, and `minimum` / `maximum` for a number. A bound declared twice is published as the intersection - the greatest lower bound, the least upper bound - since both rules run at runtime, which also makes the order they were written in irrelevant. Which keyword a size rule becomes is decided against the schema rather than the spelling of the field, so a collection reached through a type alias is still published as one; the keywords are not interchangeable, and a collection published as `minLength` reads as unconstrained to a generated client. A `nested` field publishes what the nested type declares, into the nested property (or the array's element schema), so the rule enforced on a child is visible in the schema of the child - including when the child is named by a generic parameter. Constraints are scoped to the parameters the extractor that declared them actually added, since handler arguments are described one after another into the same operation: a body and a query struct sharing a field name would otherwise hand each other their bounds, and an extractor whose schema could not be inferred - one field of an unsupported shape is enough - adds no parameters at all and must not fall back to somebody else's. A whole bound is published whole rather than as a float - signed or unsigned, so the upper half of `u64` survives too, and a bound given as a constant is read by its own type rather than guessed from the tokens - and one no JSON number can hold at all, a 128-bit bound past `u64::MAX`, is left out of the schema rather than rounded into it, since a client told a different bound sends requests the server rejects. A float bound is published at the width it is compared at, `0.7` on an `f32` field being a different number from `0.7` on an `f64` one; the width is read off the schema rather than off how the field's type is spelled, so an alias is narrowed too - since an `f64` cannot hold an integer past `2^53` without moving it while the check compares the exact value; a table that names itself - a type holding a collection of itself - is walked until the cycle closes rather than to a fixed depth. A documented constraint and an enforced one therefore cannot drift apart. The method has a default returning nothing, so a hand-written `Validate` is unaffected.
* Failures are reported under the name the client sent, not the Rust field name: `#[serde(rename = "..")]` and `#[serde(rename_all = "..")]` are read off the type, and `#[validate(rename = "..")]` overrides both - for the failure only: a constraint stays keyed by the name serde gives the property, since that is what the schema calls it. A `#[serde(flatten)]` field is reported at the level the client sent it, without the name it carries in Rust. `nested` merges a nested type's failures under `parent.child`, and a nested collection under `parent[0].child`. `ValidationError::merge` and `merge_at` do that merging and are usable by hand.
* Input validation (#218) - volga parses a payload but has never inspected it, so anything past serde (a non-empty key, a bounded page size, a range) has had to be re-written at the top of every handler. The `Validate` trait is where those rules go, and `Valid<E>` is the extractor that runs them (both unconditional - the trait is vocabulary a user or a third-party crate implements, and nothing here pulls in a dependency): it wraps another extractor, calls `validate()` on what that extractor produced, and either hands it to the handler or turns the failure into a response before the handler is entered. Volga knows none of the rules; it only knows it called the function. Because `Json`, `Query`, `Form` and `Path` all deref to their payload, one blanket impl covers all four - `Valid<Json<T>>`, `Valid<Query<T>>`, `Valid<Form<T>>`, `Valid<Path<T>>`, with `ValidJson<T>` / `ValidQuery<T>` / `ValidForm<T>` / `ValidPath<T>` as the shorthand - the last over `NamedPath<T>`, since `Path<T>` reads its payload through `FromPathArgs`, which is implemented for tuples alone, and a tuple is not a type a downstream crate can implement `Validate` for. The wrapper forwards the inner extractor's payload source, so a validated body still reads the body and a validated query still reads the parts (both may be validated in the same handler, short-circuiting at the first failure in argument order), and it forwards the OpenAPI description, so wrapping an extractor does not quietly drop the request body or the query parameters from the spec. `Option<Valid<..>>` and `Result<Valid<..>, Error>` compose as they do around any other extractor, the latter being the escape hatch for a handler that wants to shape the failure itself.
* `ValidationError` - the error `Validate` implementations accumulate into: `push(field, message)` / `push_message(message)`, `into_result()` for the collect-everything shape, `field()` / `message()` for the fail-fast one, and `entries()` to read them back. It answers `400 Bad Request`, which `with_status` overrides for a service that prefers `422`; parse failures already answer `400`, so the default keeps a malformed payload and an invalid one indistinguishable in status. Under the `problem-details` feature it renders as RFC 9457 with an `errors` extension mapping each field to the messages it collected (failures not bound to a field are grouped under an empty key), and `use_problem_details` picks that up with no extra wiring.
...
## Fixed
* Number schemas record which width they were inferred from - `format: "float"` for an `f32`, `format: "double"` for an `f64` (both registered OpenAPI formats). Previously every float published as a bare `number`, which loses a distinction a generated client needs, and left nothing able to tell how wide a bound on that field is compared.
