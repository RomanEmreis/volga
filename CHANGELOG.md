...
## Fixed
* `NamedPath<T>` read a path parameter holding a literal `&` as two fields: `/users/a&admin=true` deserialized `id = "a"` and `admin = true` into a struct with both. The `&` is kept inside the value now. (#227)
