...
## Fixed
* The index file and the fallback file are no longer served `immutable` (#230). Every static file was answered with one policy - `max-age=86400, public, immutable` - which is right for a file addressed by a content-hashed name and wrong for the two that are addressed by a stable one. `immutable` tells a browser not to revalidate even on a reload, so for the ordinary SPA layout - an `index.html` pointing at `assets/index-<hash>.js` - a user who reloaded after a deploy kept yesterday's shell for up to a day, and yesterday's shell points at asset URLs that no longer exist. The shell now defaults to `no-cache`: it is validated against the `ETag` that was always emitted, so an unchanged shell costs a `304` and no body, and a deploy is picked up on the next request instead of a day later. Assets keep the policy they had. The index file requested by its own name (`GET /index.html`) is treated as the shell too, since the name it is addressed by is stable either way.
* The index file and the fallback file answer `If-None-Match` / `If-Modified-Since` with a `304`. They are served by their own handlers rather than through the one that serves a file by name, and only the latter ran the request's validators - so `GET /` and every URL that fell back to the shell answered with a full body no matter what the client already held. Harmless while the shell was `immutable` and never revalidated; with `no-cache` it would have meant a full body on every reload, which is the cost the `ETag` exists to avoid.
* A `304` carries the `Cache-Control` it would have sent with a `200`. RFC 9111 Section 4.3.4 has a cache update the stored response from the headers of the `304`, so omitting it let a cache keep serving a file under the policy it was first stored with - a stale `immutable` among them, which is what makes this the same bug as the one above rather than a separate nicety.
* `If-Modified-Since` is compared at the precision an HTTP-date carries. A file's `mtime` has nanoseconds on any modern filesystem, an HTTP-date has whole seconds (RFC 9110 Section 5.6.7), and the two were compared as they were - so a client echoing back the very `Last-Modified` it had just been served looked strictly older than the file and was answered with a full body, every time. `ETag` validation covered this up for anything sending both validators.
* `If-Modified-Since` is ignored when `If-None-Match` is present, as RFC 9110 Section 13.1.3 requires. The two were read as an either-or, so a `304` could be answered off the date after the entity tag had already said the content changed - which is what a client sees when a deploy is rolled back to a build whose files are older than the response that client cached: it holds the newer shell, the server serves the older one, and the mismatching tag was overruled by a date that still matched.
* Conditional request headers are evaluated for `GET` and `HEAD` only, per RFC 9110 Section 13.1.3. The fallback file answers a route that was not found whatever the method was, so a conditional `POST` or `PUT` to an unknown path was answered `304 Not Modified` - a write reported as a cache hit, against an entity tag describing the shell rather than anything the request was aimed at. Only reachable at a path deeper than the segments `map_static_assets` registers; a shallower one is answered `405` by the router first.
