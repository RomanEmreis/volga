...
## Fixed
* A static file's `ETag` is derived from its bytes where the tag is actually consulted, so two versions of one file are no longer told apart by two numbers that can both stand still (#233). The tag was the SHA-1 of the file's length and the whole-second part of its `mtime`, which makes it identical for two different files whenever both of those agree - and the file this matters most for is the one where they are most likely to. A content-hashed build rewrites `index.html` as `<script src="/assets/index-a1b2c3.js">` -> `<script src="/assets/index-d4e5f6.js">`: the hash has a fixed width, so the shell keeps its byte length across deploys. What normally saved it was the `mtime` moving, which is why this was latent rather than constant - but a build that pins timestamps for reproducibility (`SOURCE_DATE_EPOCH`), a `tar -p` / `rsync -t` that preserves them, or a container layer with a fixed epoch all put two deploys in the same second by construction, and a client holding the old tag was then answered `304` for content that had changed, for as long as neither number moved again.
* **The tag a running server sends changes with no opt-in**, for the index and the fallback file. Every client currently holding one revalidates once and is answered with a body rather than a `304` - a single extra fetch of a file measured in kilobytes, per client, on upgrade. Assets are unaffected; their tag is unchanged.
* A literal segment mapped on the way to a longer route no longer hides the parameter sitting beside it. Route matching walked the tree greedily: once a segment matched a static child the walk committed to that branch and never came back, so `GET /a/{b}` stopped answering `/a/b` the moment anything mapped `/a/b/c` - `b` matched the literal on the way to the longer route, nothing was mapped where the path ended, and the request was answered `404`. The same for a path that parts ways deeper down: `/users/me/settings` beside `/users/{id}/posts` answered `404` to `/users/me/posts`. What makes this one easy to hit is that neither route has to know about the other, so mapping an unrelated longer path is enough to take a working route away - and nothing reports it, since both routes register cleanly and the parameter keeps answering every segment that is not also a literal. A lookup that reaches a dead end now unwinds to the nearest parameter it passed over and reads that segment as the parameter instead. Literals still win wherever they lead somewhere, so `/users/me` continues to beat `/users/{id}`, and a literal carrying a handler for another verb still answers `405` rather than being read as a parameter - answering the parameter route there would hand a request that belongs to the literal one to somebody else.
* The unwinding cannot run away with a request. A node in the route tree sits at one depth and is reached only after exactly that many segments have been read, so no node is visited twice within a lookup and the work stays bounded by the nodes the path can reach, rather than by the number of ways it could be read. The branch a matching request pays for this is a null check per literal step, which measures as nothing; the alternatives a walk passed over are recorded inline and only where a node holds both a literal and a parameter child.
* A bearer token that fails validation is answered `401`, not `403`. The status was hardcoded while the `WWW-Authenticate` challenge beside it was built from the token's actual failure, so a malformed, expired or wrongly signed token came back as `403 Forbidden` carrying `Bearer error="invalid_token"` - a response that contradicts itself, and one RFC 9110 does not describe, since `WWW-Authenticate` is the `401` challenge header. RFC 6750 Section 3.1 assigns `invalid_token` to `401` and keeps `403` for `insufficient_scope`, and that distinction is the whole of what the two codes tell a client: `401` says the credential can be refreshed, `403` says refreshing it will not help. A client holding a token that had merely expired was being told to stop trying. The status a rejected token carries is now the one the failure already derived and the request already held - `401` for a token that is malformed, expired, immature, wrongly signed or missing a required claim - including one that does not decode as base64, as JSON or as UTF-8, which RFC 6750 calls malformed like any other - `400` where the credential is not a bearer value at all and the request rather than the token is what has to be fixed, and `503` for a validation that could not be completed, a verification key this server cannot read among them - while a token that is valid but does not carry the role or permission a route asks for is unchanged and stays `403` with `insufficient_scope`.
* **A rejected token's status changes with no opt-in.** A client that branches on `403` where the token itself was at fault behaves differently after upgrading; one that reads the `WWW-Authenticate` challenge it was already being sent sees the status that challenge has always named. Authorization failures - the right token, the wrong role - are untouched.
* Creating a request's dependency-injection scope no longer copies the registrations. A server builds one scope per request, before it knows whether the request resolves anything at all, so the cost is paid by every request that reaches the process - and the whole registration map was rebuilt each time: a fresh `HashMap`, an entry copied for every service, an `Arc` for the map on top. Only a scoped service has anything to keep per scope; a singleton hands every scope the one instance it already holds and a transient keeps nothing between resolutions. Registrations are now shared between a container and every scope created from it, and what a scope gets of its own is an empty cell per scoped service, numbered when the container is built. The per-request cost follows the number of scoped registrations rather than the number of registrations: a scope over fifty singletons and three scoped services is built in 20 ns instead of 404 ns, and a request that creates its scope and resolves those three scoped services spends 82 ns on it instead of 453 ns. Where nothing is registered scoped, creating a scope is a single atomic increment. Resolving is unchanged at about 3 ns, scoped or not, and so is what a scope means: each still builds its scoped services once and keeps them to itself, and a clone of a container is still the same scope rather than a new one.
* `Dc<T>` borrows the container out of the request extensions instead of cloning it. The borrowing conversion was already there and the owning one was being used, so every `Dc<T>` argument of every handler paid for an `Arc` clone and its drop to read something it only ever reads.
* A dependency cycle among services no longer hangs a worker thread or takes the process down. A scoped service whose factory resolved itself - directly, or through others that lead back to it - re-entered its own cell while that cell was being initialized, and `OnceLock` deadlocks there: the request never completed and the thread serving it never came back. The same loop through transient services recursed until the stack overflowed, which aborts the whole server. The container now keeps track, per thread, of the services it is in the middle of constructing, and a construction that leads back to one of them panics with the loop spelled out (`A -> B -> A`) - a bug in the program, reported the way `core::cell::OnceCell` reports reentrant initialization. What counts as the same service is the type together with the registrations it is being built out of: a factory that resolves that type from a container of its own is not read as a loop, while one that goes through a scope of the container it is being built in still is, since a scope carries the registrations it was created from. Only construction is watched: resolving a singleton, or a scoped service already built in its scope, does not touch it.
* `Container::resolve` clones a shared instance where it lies, instead of taking a handle on it first. It went through `resolve_shared`, which bumps the `Arc` holding a singleton - or a scoped service already built in its scope - only for `resolve` to clone the value out and drop the handle again. On one thread that costs next to nothing; on many it is a write every thread resolving the service makes to the same cache line, and it serialized them: resolving a plain-data singleton from eight threads at once measured 190 ns per call, and now measures 2 ns, with nothing written to shared memory at all. `HttpContext::resolve` goes through it, and so do signed and private cookies, which resolve their key on every request that reads or writes one - and which also took the request's container by value to do it, a clone that writes the registration map's count shared by every request in flight. They borrow it now: resolving the key under the same eight-way contention measured 270 ns with a cloned container and 4 ns with a borrowed one. `resolve_shared` and `Dc<T>` are unchanged, since handing out an `Arc` is what they are for.
* A transient resolved through `resolve` is handed over as its factory built it, not cloned from it. The instance was built for that call alone and dropped right after the copy was taken, so the copy bought nothing - for a transient whose `Clone` allocates it was a second allocation per resolution (a 256-byte buffer: 51 ns down to 35 ns). One behavioural edge: a transient whose `Clone` or `Drop` has side effects now runs each of them one time fewer per resolution.
* Passing a request through the middleware chain no longer allocates for stages whose only work is to hand it on. The global chain ended in a middleware of its own, registered whenever any global middleware was, that did nothing but pass the request to the route: one more future of about 1.5 KB wrapped around the route's, allocated per request - which is why the first global middleware cost twice what each one after it did. A route's handler sat behind a middleware wrapper as well, which boxed a future around the handler's own boxed future and cloned the `Arc` holding the handler to do it; with the `middleware` feature on, that was every route, including one with no middleware at all. Both ends of a chain were also handed a placeholder `next` they never called, cloned for every request from one `Arc` shared by every chain in the application, and a route's pipeline cloned its head once more before calling it. The end of a chain is now a `NextFn` of its own rather than one more middleware: the global chain calls straight into the route's, and the route's straight into the handler. A handler with no state of its own - a function, or a closure capturing nothing - is copied into its future, which is free and touches no reference count at all; one that captures state is reached through its `Arc` as before, since cloning what it captured could cost anything. Measured over routing, the middleware chain and the handler, without the connection around them: a route with no middleware takes 215 ns instead of 274 ns and three allocations instead of four; one global middleware 257 ns instead of 368 ns, with 2 KB allocated instead of 4 KB; four of them 367 ns instead of 477 ns; a route `filter` 261 ns instead of 325 ns. What runs, and in which order, is unchanged.
* The CORS middleware clones the policy that applies to a request once, where it cloned the default policy first and then again whichever one applied.
* `TestServer` hands back a server that is already listening. It bound its socket inside the task it spawns, while the signal a test waited on was sent before `run` was called - so what a test got back was a port that something was about to listen on, and a first request that beat the bind was refused. It showed up as a `ConnectionRefused` in a test that had nothing to do with sockets, on the machine that happened to be slow that day. The port is now bound before the server task is spawned and handed to it, which also closes the window between picking a free port and taking it.
* A shutdown that arrives while a request is being handled waits for the open connections again (#217). Waiting for them took the graceful-shutdown handle back out of the environment every connection shares, and `run` could only do that by owning the environment outright - `Arc::into_inner`. Connections hold no strong reference to it, but a request being handled does, for as long as its handler runs, so a shutdown that caught one in flight skipped the wait entirely: not for that request alone but for every connection the server had open, with `run` returning while their responses were still being written. It is a race, so it showed up as the occasional truncated last response - and in exactly the case graceful shutdown exists for, a server that is busy when told to stop. Whatever drops the runtime once `run` returns - `run_blocking`, or an embedder awaiting `run` inside a `block_on` of its own - took those connections with it. The accept loop now owns the handle itself and hands each connection only what watching it needs, so the wait no longer depends on who holds the environment at that moment. A connection accepted just before the loop breaks is waited for as well, even if its task has not started yet; one still in the middle of a TLS handshake has sent no request and is dropped rather than waited for, as before. The environment - singletons included - is released once the wait is over, rather than whenever the last handler that happened to hold it returned.
* HTTPS redirection works with the `http2` feature enabled. The plain-HTTP listener was built to speak HTTP/2 alone whenever `http2` was on - `full` included, and whether or not `http1` was on beside it - so a browser, which arrives at a plain-HTTP port with HTTP/1.1, got a connection it could not use rather than a redirect. It now tells the two protocols apart, as the HTTPS listener already did.
* HTTPS redirection and HSTS read the host a request is addressed to from its target as well as from its `Host` header. Both read the header alone, and HTTP/2 never sends one: the host travels as the `:authority` pseudo-header, which lands in the request URI (RFC 9113 Section 8.3.1). So every HTTP/2 request to the redirection listener was answered `404` for having no host, and over TLS - where a browser negotiates HTTP/2 - `HstsConfig::with_exclude_hosts` never matched, and the `Strict-Transport-Security` header went to the hosts it was configured to spare. The authority an HTTP/1.1 request carries in an absolute-form target takes precedence over `Host`, as RFC 9112 Section 3.2.2 has it. A redirect to an IPv6 host no longer loses part of the address when the `Host` header names no port.
* An HSTS-excluded host is matched by its name alone, whatever port it is reached on. The comparison dropped only `:443` and `:80`, so `with_exclude_hosts(["example.com"])` did not cover `example.com:8443`, while a browser keeps an HSTS policy per host name and applies it on every port - a host served on a second port was sent the header anyway and ended up pinned to HTTPS. A port written in the list is ignored now as well, as is a userinfo in the request's authority.
* **An excluded host that names a port now covers the host on every port**, since `example.com:8443` in the list reads as `example.com`. A list that relied on the port to keep one of two ports of a host excluded sends the header to neither.
* Excluded hosts read from a configuration file are normalized like the ones set in code. Normalizing happened in `HstsConfig::with_exclude_hosts`, which a `[tls]` section deserialized into `TlsConfig` never passes through, so `exclude_hosts = ["example.com."]` or `["example.com:443"]` in a file was stored as written and matched nothing. It now happens where the configuration becomes the server's, whichever way it was built - and the matching itself moved there too, next to the normalizing it depends on, rather than living apart from it in the request pipeline.
* **A request the redirection listener cannot find a host in is answered `400`, not `404`.** RFC 9112 Section 3.2 answers an HTTP/1.1 request that lacks `Host`, carries it more than once, or carries an invalid value with `400 Bad Request`, and a redirect has nowhere to point without one - the request is what has to be fixed, not the path.

## Added
* `ETagSource` (`Metadata` / `Content`) with `HostEnv::with_asset_etag`, `HostEnv::with_shell_etag` and the matching getters - where a static file's `ETag` comes from, configured by the role of the file exactly as `Cache-Control` already is. The defaults answer the two roles the way they ask to be answered, and no deployment has to be reconfigured for the fix above to apply:
  * **assets** (`ETagSource::Metadata`, unchanged) are served `immutable`, so a client never revalidates one and never asks after its tag. Reading every asset once per deploy to derive a tag nothing consults is the cost this default declines - and it is the large files that sit here, so this is what keeps a content root full of video and bundles from being hashed end to end. Narrow `CacheControl::ASSET` until the assets do revalidate and `with_asset_etag(ETagSource::Content)` is what makes the answer trustworthy again.
  * **the shell** (`ETagSource::Content`, new) is served `no-cache`, so its tag decides between a `304` and a full body on every navigation, and it is the file whose byte length a build holds still. It is also small by construction, so what the read costs is one pass over a few kilobytes per deploy. `with_shell_etag(ETagSource::Metadata)` restores the previous derivation for a deployment that never rewrites the shell in place.
* A tag derived from a file's contents is remembered against that file's length, modification time and whatever the platform reports about the file itself, so a version is read once rather than once per request. The cache is bounded and process-wide, and it is validated more finely than a tag is derived: whole seconds are all a tag may carry, since a sub-second `mtime` differs between replicas of one build and the tag has to agree across them, while an entry in this cache is never compared against anything outside its process and uses every digit the filesystem reports. On top of those it takes the inode and the inode change time on Unix - the kernel stamps the latter on every write and on every metadata change, and no call sets it, so a deploy that pins timestamps is seen whether it renames a new file into place or rewrites the old one through. Windows has neither available on stable (`file_index` is behind `windows_by_handle`, the change time behind `windows_change_time`), so it takes the creation time instead, which NTFS file system tunneling restores to a file renamed into place under the same name within about fifteen seconds - inside that window the check there falls back to the length and the modification time. A deploy that replaces the binary or the container starts from an empty cache regardless.
* CI builds, lints and tests on Windows and macOS as well as Linux. Every job ran on `ubuntu-latest`, so no `#[cfg(windows)]` arm in this workspace had ever been compiled - the shutdown signal set, the default bind address, the default dev-certificate names, and now the discriminators above - and the `#[cfg(target_os = "windows")]` tests written for them had never run. Clippy runs there too, since a lint inside an arm Linux does not compile is a lint nothing in CI has seen.
* The worst error messages in the framework are volga's own text rather than rustc's (#221). `FromRequest`, `FromRequestRef`, `IntoResponse` and `GenericHandler` carry `#[diagnostic::on_unimplemented]`, so a handler argument that is not an extractor no longer answers with "the trait bound `NotAnExtractor: args::FromPayload` is not satisfied" - a trait that is `pub(crate)`, absent from the documentation, impossible to implement from outside the crate, and silent about what the user should have written instead. What comes back now names the built-in extractors, the wrappers that go around them, and the seam a type of your own actually reaches a handler through - inside `Json<T>` / `Query<T>` / `Form<T>`, or behind the `FromPathArgs` of a `Path<T>` and the `FromHeaders` of a `Header<T>` - rather than as an extractor of its own. The same for a handler whose return type is not a response, and for something passed to `map_get` that is not a handler at all.
* `#[http_header]` rejects a struct that is not unit-like, which its own documentation has always said it would. `#[http_header("x-api-key")] struct ApiKey { inner: String }` used to compile: the expansion never looked at the fields and handed the struct `from_static` / `from_bytes` / `new` / `from_ref` constructors that quietly ignored them. The name is the whole of a typed header - the value it carries lives in `Header<T>` - so a field there is read by nothing, and saying so at the point it is written is the only place it can be said.
* `#[derive(Claims)]` rejects an enum or a union. It used to accept anything and emit `impl AuthClaims for X {}`, which compiles and does nothing, since every method of the trait has a default. A JWT payload is a JSON object, so a struct is the only shape that could carry `role`, `roles` or `permissions` - and a struct carrying none of the three stays valid, since that is a claims type with no authorization data rather than a mistake.
* Every compile error that is part of volga's contract is locked in by a test. `trybuild` covers the two derives, the attribute macro, and the four traits above - `volga/tests/ui/` is one directory per contract - and only messages this crate writes itself are asserted, since rustc's own wording moves between toolchains and this repository builds on two. The two `compile_error!` guards that require an HTTP transport cannot be reached that way, since a `trybuild` case is built against the feature set the outer test run resolved; they are checked by a CI step that builds `volga` and `volga-oauth-client` with `--no-default-features` and expects the failure to carry the guard's message.
* `ContainerBuilder::validate` checks the dependency graph before anything is resolved, and `App::run` calls it on startup: a dependency cycle, or a service depending on a type nothing registered, now fails `run` before the first connection is accepted - the channel that already reports a bearer configuration naming both a static key and an OAuth issuer. It fails before `run` has started anything of its own, too: no shutdown trigger, no signal watcher and no HTTPS redirect listener, so an app that refuses to start leaves no task running and no second port taken, and a caller that handles the error and tries again finds them free. The greeter and the `listening on` line wait for the same point, so a start that fails no longer announces a listening server to an operator, or to a readiness check reading the log. The graph is what the registrations declare: a factory's arguments (`add_scoped_factory(|db: Dc<Db>| ...)` depends on `Db`), and `Inject::dependencies`, a new provided method a hand-written `Inject` implements to say what its `inject` resolves, collected into `Dependencies`. The default declares nothing, so no existing implementation changes meaning; a type that declares nothing is left out of the startup check and falls to the check made while constructing it. Every problem is reported at once, in a stable order, rather than the first one found.
* **A registration whose declared dependency is not registered now stops the app at startup.** It used to fail every request that resolved it with a `500`, and was silent for as long as nothing did - so an application that carries such a registration unused has to register the dependency or drop the registration.

## Notes
* The tag stays **weak** whichever source it came from. RFC 9110 Section 8.8.1 reserves strong validation for octet-equality of the representation that is actually sent, and the compression middleware may re-encode a body after the static file server has set the header - so a strong tag would be a promise this server is not in a position to make. This is where `actix-files` and `ntex-files` differ, emitting `ino:len:secs:nanos` as a strong tag; `tower-http`, which is what `axum` serves static files with, emits no `ETag` at all and validates on `Last-Modified` alone.
...
* A route group applies its configuration to the routes it registered, whatever the order inside the closure (#231). It used to read its middleware, CORS policy and OpenAPI config at each `map_*`, so a `g.with(require_api_key)` written below a route silently did not reach it - and what a group carries is usually authorization, rate limiting or CORS. The group now applies what it holds once its closure returns. Middleware still runs in registration order, and a group's still runs before that of a route or a nested group inside it. A sub-group inherits the parent's configuration wherever it is declared, as `RouteGroup::group` has always documented, and a CORS policy a route or a nested group chose is no longer replaced by the enclosing group's.
* A route is registered under the name the router reads, so `/x`, `/x/` and `//x` are one route everywhere it is remembered rather than only in the route tree. Each registration used to keep the string it was written as, and that string keys the group's record of the route and its OpenAPI operation, so one route could be several entries there - a group prepending its middleware once per spelling, or an operation described by whichever registration hash order reached last. A group configures each of its routes once, whether it mapped it twice itself or a sub-group mapped it again. Two spellings of one *dynamic* route (`/users/{id}` and `/users/{name}`) still count as two: that is an ambiguity to report, not to hide here.
* Mapping a route that is already mapped replaces it, along with everything bound to the registration being replaced. Both used to land on one endpoint and the result was neither: the handler mapped last answered while the middleware of the one mapped first ran, and the middleware written next to the answering handler did not. A handler and its middleware are written together and now stand or fall together. The OpenAPI operation follows the same rule - it is built from the configuration the route currently holds, instead of keeping fields, such as a summary, written for a handler that has since been replaced.
* A `HEAD` request no longer walks past the middleware guarding the `GET` it answers. `map_get` used to register a second route under `HEAD` carrying the handler and nothing else, so route middleware, group middleware and the route's CORS policy went to the `GET` alone: a route behind a group's `authorize` answered `403` to `GET /admin/report` and `200` to `HEAD /admin/report`, with rate limiting and filters skipped along with it. A `GET` route is no longer mapped twice - routing hands a `HEAD` request that has no route of its own to the `GET` route - so it travels through everything that route travels through, and the body is dropped on the way out as before. `map_head` still takes precedence and runs its own middleware and none of the `GET` route's.
* Two names for one route parameter are told apart, instead of one of them being swallowed (#226). A node in the route tree holds a single dynamic child, because a parameter is matched by the position it sits at and not by what it is called - and that child used to hold the name as well, the one whichever route reached the position first was written with. Every other route through that position then labelled its request with that name: `POST /users/{name}` mapped beside `GET /users/{id}` answered correctly but bound its parameter as `id`, so `NamedPath<T>` and everything else reading a parameter by name read a key nobody wrote, and the startup route listing printed `POST /users/{id}`. Each endpoint now carries the names its own pattern was written with, and the request reaching it is labelled from those - so the two routes above bind `id` and `name`, and are listed as they were written. Positional extractors (`id: i32`, `Path<(A, B)>`) never read the name and are unchanged, and a route agreeing with the tree - nearly all of them - stores nothing and costs a request nothing.
* Two cases cannot be told apart that way, and are now reported where they are written rather than swallowed. The first is one verb naming its own route twice - `map_get("/users/{id}")` followed by `map_get("/users/{name}")` - where the second registration replaces the first and takes its middleware with it: one route ends up mapped rather than two, and the differing name says that was not the intention. The second is a `GET` and a `HEAD` disagreeing: a `HEAD` request with no route of its own is answered by the `GET` route (RFC 9110 Section 9.3.2), so the two describe one resource and cannot name what identifies it differently. Both panic at registration, naming the two patterns with their verbs and what to write instead. Any other verb may name the position whatever it likes, and two routes on one verb that part at a position - `/users/{id}/posts` beside `/users/{name}/comments` - never meet at an endpoint and keep their own names too.
* The startup banner closes its border on the same column on every line, whatever the version and the address are. The width was written into the border literals and the version padded to five columns, so `0.10.0` pushed the right edge one column out and an address longer than the box allowed for did the same. The box is sized from what it holds now - the same 47 columns by default, wider when a long version or an IPv6 address needs the room.
* `cargo clippy --all-targets --no-default-features --features full -- -D warnings` passes on the repository itself (#238). Four fixtures in the `validation-derive` integration test are used only by tests gated on `test` and `openapi` but carried no gate of their own, so a build with `validation-derive` and without `test` - `full` is one - compiled them as dead code. They carry the gate their only users carry now, and the lint job checks the `full` feature set, which is the shape neither `--all-features` nor `--features http1` / `http2` compiles.

## Changed
* The static file server is middleware rather than routing (#228). `use_static_files()` used to walk the content root at startup, register one route per level of the tree it found, and rebuild the request path from the parameters those routes bound. It now registers a single middleware that reads the request target, answers it from the content root when something is there, and declines otherwise. What the old shape cost goes with it: no startup walk, no depth ceiling - a directory created while the server runs is served like any other - no path to reassemble, and nothing static in the router, so none of it reaches the route listing or an OpenAPI spec and none of it claims the router's single dynamic slot: `use_static_files()` and `map_get("/{id}", ..)` now coexist (#226). What the files are served *with* is unchanged - `ETag`, `Last-Modified`, the `Cache-Control` policies of `HostEnv`, the directory listing, the `403` on a directory with listing off, and `map_fallback_to_file` - so an SPA shell behaves exactly as before.
* Where the static files sit in the pipeline is now the position of the `use_static_files()` call: after `use_compression()` to have the files compressed, after `use_cors()` to have the headers on them, before anything that should not run for a request answered from disk.
* The request target is resolved from its ordinary path components alone: a `.`, a `..`, an encoded separator (`%2F`) or an embedded NUL is declined rather than dropped or looked up, so traversal is refused by construction instead of being caught after the fact. The `canonicalize` check behind it stays, and still answers `403`, for the case a request target cannot describe: a symlink under the content root pointing out of it.
...
* `App::use_static_assets` and `RouteGroup::use_static_assets` - the static file mount on its own, without the fallback file `use_static_files` adds. Renamed from `map_static_assets`, which no longer maps anything.

## Breaking Changes
* `map_static_assets` is renamed to `use_static_assets` (#228): there is no route left to map, and `map_*` in this crate means a route was registered. `use_static_files` keeps its name and its meaning - the mount, plus `map_fallback_to_file` when a fallback file is configured.
* A file answers before a route does. The mount short-circuits a `GET` or `HEAD` whose target names something under the content root, so a route mapped for a path that is also a file on disk no longer answers it - the other side of #226, and the price of the router no longer being involved. Everything else, another method or a path with nothing behind it, reaches routing as before. A group prefix - `app.group("/static", |g| g.use_static_files())` - keeps the mount to one part of the URL space.
* The `static-files` feature implies `middleware`. Enabling the first brings the second in, since that is what serves the files now.
* A group whose prefix carries a route parameter - `app.group("/{tenant}", |g| g.use_static_files())` - no longer serves static files, and says so on startup. A mount matches the request target as it is written, while a parameter is matched by the router, which knows nothing about the mount. What that spelling used to do was an accident of the removed path reassembly: every parameter the route bound was folded into the filesystem path, so `/{tenant}` served `<content root>/<tenant>/<file>`. Mount under a literal prefix; #239 tracks the feature itself.
* A group's CORS policy no longer reaches the files that group serves; the application's does, since a policy is resolved from the route that matched and a file is served without one. The group's middleware does still reach them: `wrap`, `with`, `filter`, `map_ok` and the rest wrap what the mount answers with, nested groups included. For the same reason a preflight aimed at a file's path is answered as one for an unmatched path - a `404` carrying the policy's headers rather than a `204` - which cross-origin `GET`s of static content never send anyway.
...
* Middleware that used to be skipped now runs, with no opt-in. A route mapped before its group's `authorize` or `token_bucket` behaved as if neither existed, and a `HEAD` request reached the handler through none of it; both now answer `401` / `403` / `429` where they answered `200`. A `HEAD`-based health check against a route behind `authorize` is the likeliest thing to notice.
* `App::without_implicit_head` is removed. It stopped `map_get` from registering a second route under `HEAD`, and there is no second route to stop; what it turned off was `HEAD` support, which RFC 9110 Section 9.1 requires of a general-purpose server. A service that wants `HEAD` on some path to fail can map one that says so.
* Mapping the same route twice is now decided by the last registration rather than by a mix of both. This is wrong code either way, and what changes is that its result is the one a reader would predict.
* The `RouteGroup::<method> must be called before any map_* in the group` warning is gone: it described a hazard that no longer exists. A test asserting on that output has nothing to assert.
* An application whose routes disagree in one of those two ways panics at registration instead of starting with a route silently missing (#226). Two verbs naming one position differently now start and route correctly, so what stops starting is narrower than it sounds: one verb mapped twice under two names, and a `HEAD` mapped apart from the `GET` it answers for. A group prefix counts the same way, being a route pattern like any other - `group("/{tenant}", ..)` beside `map_get("/{id}/items", ..)` is one route under two names.
* A parameter read by name on a route that another verb reached first is bound under the name that route was written with, where it used to be bound under the other one's. `NamedPath<T>` in a `POST /users/{name}` handler mapped beside `GET /users/{id}` used to fail deserializing, having been handed `id`, and now finds `name`; a handler written around the old behaviour - reading `id` from a route that says `{name}` - reads nothing. Positional extractors never looked at the name and are unaffected.

# 0.9.11

## Added
* `HostEnv::with_asset_cache_control` and `HostEnv::with_shell_cache_control` (#230) - the static file server's `Cache-Control` is now configured where the rest of the static file server is configured, rather than being a constant. Each takes the policy currently in effect and returns the one to use, so narrowing a single directive does not mean restating the others. `HostEnv::asset_cache_control` and `HostEnv::shell_cache_control` read them back, and the two defaults are named by the `CacheControl::ASSET` / `CacheControl::SHELL` constants for anyone building one from scratch.
* `CacheControl::asset()` and `CacheControl::shell()` - the same two defaults as ready `Header<CacheControl>` presets, for a handler attaching one to a response rather than configuring a server. They sit with `no_cache()`, `public()` and the rest in `known_headers`, under the `static-files` feature that gives them their meaning. `shell()` renders what `no_cache()` does and is deliberately not an alias: one names a directive, the other names the role the directive was chosen for.
...
* `CacheControl::EMPTY` - the `const` equivalent of `CacheControl::default()`.

## Fixed
* The index file and the fallback file are no longer served `immutable` (#230). **The header a running server sends changes with no opt-in:** `GET /` answered `max-age=86400, public, immutable` and now answers `no-cache`. Every static file used to get that one policy, which is right for a file addressed by a content-hashed name and wrong for the two addressed by a stable one - `immutable` tells a browser not to revalidate even on a reload, so for the ordinary SPA layout, an `index.html` pointing at `assets/index-<hash>.js`, a user who reloaded after a deploy kept yesterday's shell for up to a day, and yesterday's shell points at asset URLs that no longer exist. The shell is validated against the `ETag` that was always emitted, so an unchanged one costs a `304` and no body, and a deploy is picked up on the next request instead of a day later. Assets keep the policy they had, `GET /index.html` is treated as the shell since the name it is addressed by is stable either way, and the previous behaviour is one call away: `with_shell_cache_control(|cc| cc.with_max_age(86400).with_public().with_immutable())`.
* Conditional requests on static files were mishandled in five ways. Each was survivable while the shell was `immutable` and never revalidated; with `no-cache` the shell is revalidated on every load, so all five sit on the hot path the change above creates:
  * The index file and the fallback file ignored `If-None-Match` / `If-Modified-Since` entirely. They are served by their own handlers rather than through the one that serves a file by name, and only the latter ran the validators - so `GET /` and every URL that fell back to the shell answered with a full body no matter what the client already held.
  * `If-Modified-Since` was compared against a nanosecond `mtime`. An HTTP-date carries whole seconds (RFC 9110 Section 5.6.7) and a file's `mtime` does not, so a client echoing back the very `Last-Modified` it had just been served looked strictly older than the file and got a full body, every time. `ETag` validation covered this up for anything sending both validators.
  * `If-Modified-Since` was read even when `If-None-Match` was present, which RFC 9110 Section 13.1.3 forbids. The two were an either-or, so a `304` could be answered off the date after the entity tag had already said the content changed - what a client sees when a deploy is rolled back to a build whose files are older than the response that client cached.
  * Validators were evaluated whatever the request method was. The fallback file answers a route that was not found on any method, so a conditional `POST` or `PUT` to an unknown path was answered `304 Not Modified` - a write reported as a cache hit, against a tag describing the shell rather than anything the request was aimed at. Only reachable below the depth `map_static_assets` registers; a shallower path is answered `405` first.
  * A `304` carried no `Cache-Control`. RFC 9111 Section 4.3.4 has a cache update the stored response from the headers of the `304`, so omitting it let a cache keep serving a file under the policy it was first stored with - a stale `immutable` among them.
* `App::with_cache_control` documents that it does not reach the static file server, which sets the header itself. That was already true, and the previous wording - "if they are not explicitly configured for the route or route group" - suggested otherwise; the static files' policy is configured on `HostEnv`.
...
* Registering a group-level setting after a route in that group is reported instead of silently doing nothing (#231). A `RouteGroup` reads its middleware, CORS policy and OpenAPI configuration at the moment each route is mapped, so a `wrap` / `with` / `attach` / `filter` / `map_ok` / `map_err` / `tap_req` / `cors` / `cors_with` / `disable_cors` call placed after a `map_*` applies to none of the routes above it - and the failure mode is quiet, since what those calls usually carry is authorization, rate limiting or a CORS policy, and a route that never received one looks exactly like a route that was not meant to have one. `RouteGroup::open_api` has warned about this since it was added; every other group-level setting now warns the same way, naming the method, the group's prefix and how many routes it came too late for. Routes mapped by a nested `group` count towards the parent's total as well, so a parent registering middleware after a sub-group - which inherited only what the parent held when it was created - is reported too. A group-level helper reports itself rather than the primitive it is built on - `token_bucket`, `fixed_window`, `sliding_window`, `gcra`, `authorize`, `with_compression`, `with_decompression`, `cache_control` and `map_problem` all end up in one of `attach` / `wrap` / `map_ok` / `map_err`, and a warning naming those would point at a method the caller never wrote. The warning goes to `tracing` when the feature is on and to `stderr` otherwise. This is a diagnostic and nothing else: when a group's configuration is captured is unchanged, and turning the group into a scope whose configuration applies to every route it registered, whatever the order, is a breaking change left for 0.10.
...
## Added
* Input validation (#218) - volga parses a payload but has never inspected it, so anything past serde (a non-empty key, a bounded page size, a range) had to be re-written at the top of every handler. The `Validate` trait is where those rules go, and `Valid<E>` is the extractor that runs them: it wraps another extractor, calls `validate()` on what that extractor produced, and either hands it to the handler or turns the failure into a response before the handler is entered. Volga knows none of the rules; it only knows it called the function. Both are unconditional - the trait is vocabulary a user, or a third-party crate, implements, and nothing here pulls in a dependency. `Json`, `Query`, `Form` and `NamedPath` all deref to their payload, so one blanket impl covers them all, with `ValidJson<T>` / `ValidQuery<T>` / `ValidForm<T>` / `ValidPath<T>` as the shorthand. The wrapper forwards the inner extractor's payload source, so a validated body still reads the body and a validated query still reads the parts - both may be validated in the same handler, short-circuiting at the first failure in argument order - and it forwards the OpenAPI description, so wrapping an extractor does not quietly drop the request body or the query parameters from the spec. `Option<Valid<..>>` and `Result<Valid<..>, Error>` compose as they do around any other extractor, the latter being the escape hatch for a handler that wants to shape the failure itself.
* `ValidationError` - the error `Validate` implementations accumulate into: `push(field, message)` / `push_message(message)` with `into_result()` for the collect-everything shape, `field()` / `message()` for the fail-fast one, `entries()` to read them back, and `merge` / `merge_at` to fold one error into another. It answers `400 Bad Request`, which `with_status` overrides for a service that prefers `422`; parse failures already answer `400`, so the default keeps a malformed payload and an invalid one indistinguishable in status. A status asked for by a merged failure is kept, so `with_status` works from a `schema` or `custom` function as it does from a hand-written impl. Under the `problem-details` feature it renders as RFC 9457 with an `errors` extension mapping each field to the messages it collected (failures not bound to a field are grouped under an empty key), and `use_problem_details` picks that up with no extra wiring.
...
* `#[derive(Validate)]` (feature `validation-derive`) - writes out the same `validate()` a hand-written impl would, from the bounds declared on the fields. It is not a second way to validate: the trait stays the seam, so a derived impl and a hand-written one are indistinguishable to `Valid<E>`. Field rules are `length(min, max, equal)` for strings and collections, `range(min, max)` for anything ordered, `nested` for a field validating itself, and `custom = "path::to::fn"`, each taking an optional `message = ".."`. A rule spanning two fields cannot live on either of them, so the container takes `schema = "path::to::fn"` for those - which is also why the derive is sugar over the trait rather than a replacement for it. All failures are collected rather than stopping at the first, and every bound and message is rendered at expansion time, so a failing check pushes a `&'static str` and allocates nothing. An attribute the derive could only half-honour is refused at compile time rather than half-applied: `equal` alongside `min` or `max`, since `equal` decides the check on its own, and a `range` bound that is neither a literal nor a constant, since a bound is read once when the route is described and again on every request - anything that could answer differently between the two would publish a contract the server does not enforce. A rule that genuinely varies belongs in `custom`.
* Failures are reported under the name the client sent, not the Rust field name: `#[serde(rename = "..")]` and `#[serde(rename_all = "..")]` are read off the type, and `#[validate(rename = "..")]` overrides both - for the failure only, since a constraint has to stay keyed by the name the schema gives the property. A `#[serde(flatten)]` field is reported at the level the client sent it, without the name it carries in Rust. `nested` merges a child's failures under `parent.child`, and a nested collection under `parent[0].child`.
* The declared rules are published as well as enforced: `Validate::constraints()` reports what the fields declare, and `Valid<E>` forwards it into the OpenAPI description, where it lands on the property of the request schema or on the parameter that same extractor added - never on one belonging to another handler argument. Which keyword a size rule becomes is decided against the schema rather than the spelling of the field, so `minLength`, `minItems` and `minProperties` each describe what they should even when the type is reached through an alias; the keywords are not interchangeable, and a collection published as `minLength` reads as unconstrained to a generated client. A `nested` field publishes what the nested type declares, into the nested property or the array's element schema, generic wrappers included. A bound declared twice is published as the intersection, which also makes the order the rules were written in irrelevant. What cannot be published exactly is not published at all rather than rounded to something else: a numeric bound keeps its width and signedness, a float bound is read at the width it is compared at, and one no JSON number can hold - a 128-bit bound past `u64::MAX` - is left out. A documented constraint and an enforced one therefore cannot drift apart. The method has a default returning nothing, so a hand-written `Validate` is unaffected.
...
* Number schemas record the width they were inferred from - `format: "float"` for an `f32`, `format: "double"` for an `f64`, both registered OpenAPI formats. Every float previously published as a bare `number`, which loses a distinction a generated client needs.
* `volga::auth::permission` is re-exported alongside its four siblings (#216). The singular form was the only one of `role`, `roles`, `permission`, `permissions` and `predicate` missing from `volga::auth`, so copying the list of built-in authorizers out of the documentation did not compile, and the compiler's suggestion - `permissions` - is a different function. Purely additive.
