...
* A route group applies its configuration to every route it registered, not only to the ones mapped after that configuration was registered (#231). A `RouteGroup` used to read its middleware, CORS policy and OpenAPI configuration at the moment each `map_*` ran, so a `g.map_get(..)` followed by a `g.with(require_api_key)` left that route without `require_api_key` - and since what a group usually carries is authorization, rate limiting or a CORS policy, a route that quietly lost its protection looked exactly like a route that was never meant to have any. 0.9.11 made that a startup warning; the group now applies what it holds once its closure returns, so where a `map_*` sits inside the closure decides nothing, and the warning is gone along with the trap it described. Order still means what it always meant everywhere else: middleware runs in the order it was registered on the group, and a group's middleware runs before the middleware of a route or of a nested group inside it - it is inserted ahead of what those already put on the route rather than appended after it.
* A route is registered under the name the router reads, so two spellings of one path are one route everywhere it is remembered rather than only in the route tree. `map_get("/x")` and `map_get("/x/")` always answered as one route - empty segments route nowhere - but each kept the string it was written as, which gave one route two OpenAPI operations to be described by: the operation published for it could carry a summary written for the registration that the other one replaced, and which of the two won depended on hash order once OpenAPI was configured after the routes. The path a route is registered under, and the one its OpenAPI operation is published under, is now the one the router matches.
* An OpenAPI operation describes the route as it stands. Re-registering a route replaced the configuration it was described by but kept the operation built from the previous one, so a field the new registration says nothing about - a summary, a description, an `operationId` - survived from a handler that had been replaced. The operation is now built from the configuration the route currently carries.
* Mapping a route that is already mapped replaces it, along with everything bound to the registration being replaced. Both registrations used to end up on one endpoint, and what came out was neither of them: the handler mapped last answered, the middleware of the one mapped first ran, and the middleware written next to the answering handler did not - so a `map_get(..).wrap(check)` shadowed later by another `map_get` for the same path ran `check` in front of a handler it was never written for, while the second handler's own `wrap` was unreachable. A handler and its middleware are written together and now stand or fall together: the last registration wins whole. This is most visible where it is least intended - a sub-group mapping a path the parent already mapped - and it stays wrong code either way; what changes is that the result is now the one a reader would predict. The middleware of the groups the route sits in is not part of a registration and still applies.
* A route group builds a route's path the way the router reads it, so `/x` and `/x/` under the same group name one route rather than two. Empty segments never routed anywhere - `split_path` drops them - but the group kept the string it was handed, counted the two spellings as two routes and prepended its middleware once for each, spending two rate-limiting permits on one request. The path registered for such a route loses its empty segments too, which is what the OpenAPI operation and the startup route list now show, and a group standing at the root names it `/` - the spelling a route mapped outside a group uses - rather than an empty string that would address the same route under a second name.
* A route mapped twice in the same group takes that group's configuration once. The second `map_get` for a pattern lands on the endpoint the first one made, so it is one route; the group used to count it as two and prepend its middleware twice - a `token_bucket` spending two permits per request - and repeat its OpenAPI tag on the operation. A route a group and one of its sub-groups both map is the same case and counts once as well. Two spellings of one dynamic route (`/users/{id}` and `/users/{name}`) still count as two, since they are the ambiguity problem rather than this one.
* A sub-group inherits the parent group's middleware, CORS policy and OpenAPI configuration wherever it is declared, which is what `RouteGroup::group` has documented all along. A sub-group written above the parent's `with` or `cors_with` used to inherit only what the parent held at the moment the sub-group was created, which was usually nothing.
* A CORS policy chosen by a route or by a nested group is no longer at risk of being replaced by the enclosing group's. A route endpoint now tracks "no policy bound" separately from "inherit the application's default", and a group binds its policy only to the routes that bound none of their own - so `.cors()` on a route inside a group carrying a named policy means the application default, as it reads.
* A `HEAD` request no longer walks past the middleware guarding the `GET` it answers. `App::map_get` used to register a second route under `HEAD` carrying the handler and nothing else, so the middleware bound to the route, the middleware of the group it belongs to and its CORS policy all went to the `GET` alone: a route behind a group's `authorize` answered `403` to `GET /admin/report` and `200` to `HEAD /admin/report` - status and headers for a request the middleware existed to reject, with rate limiting, filters and `tap_req` skipped along with it. A `GET` route is no longer mapped twice: routing hands a `HEAD` request that has no route of its own to the `GET` route for that path, so it travels through everything that route travels through, and the body is dropped on the way out as it always was. `map_head` still takes precedence and is a route of its own - it runs its own middleware and none of the `GET` route's - and `Allow` still names `HEAD` for a `GET` route, since the resource supports it. A `HEAD` request is now answered wherever a `GET` is, with no way to turn that off: RFC 9110 Section 9.1 has a general-purpose server support `GET` and `HEAD`, and Section 9.3.2 makes the second one the first without content, so the two are not separate capabilities to opt out of.
...
* The routes that were silently missing their group's middleware now run it. That is the point of the fix above, and it changes what a running application answers: a route mapped before its group's `authorize` or `token_bucket` behaved as if neither had been registered, and now answers `401` or `429` the way the group always meant it to. Nothing needs reordering - order no longer matters - but a request that used to be served may now be rejected by middleware that was always supposed to see it, so it is worth reading each group closure once before upgrading to know which routes those are.
* `App::without_implicit_head` is removed. It existed to stop `map_get` from registering a second route under `HEAD`, and there is no second route to stop: a `HEAD` request with no route of its own is answered by the `GET` route itself. What the switch turned off was `HEAD` support, which RFC 9110 Section 9.1 requires of a general-purpose server - and a service that really wants `HEAD` on some path to fail can map one that says so. Calls to it are a compile error rather than a silent change of behaviour.
* The `RouteGroup::<method> must be called before any map_* in the group` warning is gone, and nothing logs it any more. It described a hazard that no longer exists; a test asserting on that output has nothing to assert.
* A `HEAD` request to a `GET` route that is guarded by middleware is now answered by that middleware. The implicit `HEAD` endpoint used to reach the handler directly, so a client sending `HEAD` where it used to get `200` may now get whatever the route's or the group's middleware answers - `401`, `403`, `429` - and a health check or an uptime monitor built on `HEAD` against a route behind `authorize` is the likeliest thing to notice. This is the point of the fix: `HEAD` was never meant to be a way in.
