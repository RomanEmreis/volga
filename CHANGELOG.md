...
## Changed
* The static file server is middleware rather than routing (#228). `use_static_files()` used to walk the content root at startup with a blocking `read_dir`, register `/{path_0}`, `/{path_0}/{path_1}`, ... one route per level of the tree as it stood at that moment, and rebuild the request path out of the parameters those routes bound. It now registers a single middleware that reads the request target, answers it from the content root when something is there and declines otherwise - the router knows nothing about static content, as it does not in ASP.NET Core, which is where the name came from. Everything awkward about serving static files followed from the old shape and goes with it: there is no filesystem walk at startup; there is no depth ceiling, so a directory created while the server is running is served like any other rather than answering `404` forever because the routes were counted before it existed; there is no path to reassemble; and nothing static is in the router, so none of it shows up in the route listing, none of it has to be reasoned about in an OpenAPI spec, and none of it claims the router's single dynamic slot - `use_static_files()` and `map_get("/{id}", ..)` now coexist, each answering what belongs to it (#226). What the files are served *with* is unchanged: `ETag`, `Last-Modified` and the `Cache-Control` policies of `HostEnv`, the directory listing, the `403` on a directory with listing off, and `map_fallback_to_file`, which still answers what neither a file nor a route did - an SPA shell behaves exactly as before.
* Where the static files sit in the pipeline is now the position of the `use_static_files()` call among the other middleware, and an explicit decision: register it after `use_compression()` to have the files compressed, after `use_cors()` to have the headers on them, and before anything that should not run for a request answered from disk.
* The request target is resolved from its ordinary path components alone - a `.`, a `..`, an encoded separator (`%2F`) or an embedded NUL is declined rather than dropped or looked up - so traversal is structurally impossible instead of being caught after the fact. The `canonicalize` check behind it stays, and still answers `403`, for the one case a request target cannot describe: a symlink under the content root that points out of it.
...
* `App::use_static_assets` and `RouteGroup::use_static_assets` - the static file mount on its own, without the fallback file that `use_static_files` adds. This is `map_static_assets` under the name the thing it now does deserves.

## Breaking Changes
* `map_static_assets` is renamed to `use_static_assets` (#228). It maps nothing any more - there is no route left to map - and `map_*` in this crate means a route was registered. `use_static_files` keeps both its name and its meaning: the mount, plus `map_fallback_to_file` when a fallback file is configured.
* A file answers before a route does. The mount short-circuits the pipeline for a `GET` or `HEAD` request whose target names something under the content root, so a route mapped for a path that is also a file on disk no longer answers it. That is #226 read from the other side, and it is the price of the router no longer being involved: everything else - another method, a path with nothing behind it - reaches routing exactly as before. Mounting the files under a group, `app.group("/static", |g| g.use_static_files())`, keeps them to one part of the URL space.
...
* A route group whose prefix carries a route parameter - `app.group("/{tenant}", |g| g.use_static_files())` - no longer serves static files, and says so on startup. A mount is matched against the request target as it is written, and a parameter is matched by the router, which knows nothing about the mount. What that spelling used to do was an accident of the same path reassembly this release removes: every parameter the route bound was folded into the filesystem path, so `/{tenant}` served `<content root>/<tenant>/<file>` - undocumented, untested, and one content root away from being a per-tenant one. Mount under a literal prefix instead.
* A group's CORS policy no longer reaches the files that group serves. Its middleware still does - `wrap`, `with`, `filter`, `map_ok` and the rest registered on a group wrap what the group's mount answers with, as they wrap the routes it mapped, and a nested group inherits from the groups around it - but a CORS policy is resolved from the route that matched, and a file is served without one, so a mount inside a group carrying `cors_with` gets the application-wide policy instead. For the same reason a preflight aimed at a file's path is now answered as a preflight for a path no route matched: a `404` carrying the policy's headers rather than a `204`. Cross-origin `GET`s of static content are simple requests and do not preflight, so this is visible mostly to something asking about a file it is not going to fetch that way.
