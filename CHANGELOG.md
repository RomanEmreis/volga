...
## Added
* `volga::error::IntoError`, which turns the `Err` of a handler's `Result<T, E>` into an `Error` for the error handler. It is implemented for `Error` itself and every error type volga converts into one (`std::io::Error`, `serde_json::Error`, `Infallible` and the rest), for `StatusCode` (that status, with its canonical reason as the message), for `(StatusCode, E)` where `E` is a message or any error, for `String`, `&'static str`, `Cow<'static, str>` and `Box<str>` (`500` with that message), for `Box<dyn std::error::Error + Send + Sync>` (`500`), and for `Problem<E>`. For a type of your own, `IntoError` is the one impl to write: it also gives `From<T> for Error` through a blanket impl, so `?` converts the type in a handler returning `HttpResult`, in middleware and in `tap_req`. volga's own error types implement `IntoError` now instead of `From`, and they convert with `?`, `.into()` and `Error::from` as before. `Error` itself does not implement `IntoError`: a handler's `Result<T, Error>` has an `IntoResponse` impl of its own. A type with its own `From<T> for Error` still converts with `?`, but it becomes a handler's `Err` only once the body of its `from` moves into `into_error`; keeping both impls is a conflict (E0119). There are deliberately no impls for integers: `Err(404)` could mean a status or an application's error code, and `Err(StatusCode::NOT_FOUND)` is unambiguous. An `Err` type without it is a compile error that names `IntoError`. `tap_req` keeps `Result<HttpRequestMut, Error>`, so that a `?` followed by a bare `Ok(..)` still infers the error type there. (#262)
* `Error::with_response`, `Error::has_response` and `Error::take_response` let an error carry the response it answers with, such as a JSON envelope or a `Problem`, while still going through the error handler. `map_err` receives the error and can either answer with something else or return the error to use the attached response. The default handler and `use_problem_details` answer with the attached response unchanged. The response is sent with the error's own status, so a bare body such as `Json(..)` can be attached. (#262)
* `IntoError::describe_openapi` (feature `openapi`) lets an error type describe its own responses in a route's OpenAPI operation. `Result<T, E>` now describes both `T` and `E`. The default describes nothing, since the error handler decides what an error looks like. (#262)
* The WebSocket message conversions accept errors of their own type. `map_msg`, `WebSocket::on_msg`, `WebSocket::send` and `recv`, and `WsSink::send` and `WsStream::recv` take any `TryInto<Message>` or `TryFrom<Message>` whose error converts into a `volga::error::Error`, which any `IntoError` type does. Previously the error had to be `volga::error::Error`. As a result, a `map_msg` handler can now reply with a `Message` directly. (#262)
* A `custom_error` example: an error enum with a JSON body of its own, filled in by `?`.

## Changed
* **Breaking:** the `Err` of a handler's `Result<T, E>` is now treated as an error, not as a second response. Before, `E: IntoResponse` rendered the `Err` exactly as it rendered the `Ok` value. So `Err(String)` answered `200` with the string, and `Err(StatusCode)`, `Err(Problem)` or `Err(HttpResponse)` produced a response directly, bypassing `map_err` and `use_problem_details`. Now `E` must implement `IntoError`, and the error goes to the error handler. This applies wherever a `Result` is answered: handlers, `map_err`, `map_fallback`, and the `with` and `map_ok` middleware. On upgrade:
  - `Err(String)`, `Err(&'static str)`, `Err(Cow<'static, str>)` and `Err(Box<str>)` now answer `500` with the message instead of `200`, and they reach `map_err`. This code still compiles, so nothing flags the change.
  - `Err(StatusCode)` keeps its status but now reaches `map_err`. Its body is the canonical reason as `text/plain` instead of empty, or `application/problem+json` under `use_problem_details`.
  - `Err(Problem)` answers as before, but reaches `map_err` first.
  - An `Err` of `HttpResponse`, `HttpResult` (as in `Err(status!(..))`), `Json<T>`, `Form<T>`, `()` or any other response type no longer compiles. Return it as the `Ok` value instead, or attach it to an error with `Error::with_response`.
* The compile error for an invalid handler return type now mentions `IntoError` as the requirement for a `Result`'s `Err`.
* `Error` keeps its instance and any attached response out of line, allocated only once one of them is set. This shrinks `Error` from 48 bytes to 32 on 64-bit targets, and every extractor's `Result<T, Error>` shrinks with it when `T` is no larger. `HttpResult` stays the same size as `HttpResponse`. (#262)

# 0.11.2

## Added
* `volga::openapi::OpenApiSchema`, also exported from `volga-open-api`. It was public but unreachable, so `with_request_schema` and `with_response_schema` could not be called from outside the crate.
* `OpenApiRouteConfig::with_query_schema` describes query parameters by hand from an object schema: one parameter per property, required as the schema's `required` list says, replacing a parameter of the same name. (#220)
* `OpenApiRouteConfig::undescribed_inputs` lists the handler inputs an operation describes without their fields, as `UndescribedInput`s (see Fixed). Describing one by hand removes it. (#220)
* `OpenApiRegistry::describe_route_as` and `OpenApiRegistry::remove_position`, the registry primitives behind routes that name one position differently (see Fixed). (#253)

## Fixed
* A type with a `#[serde(flatten)]` field was published with none of its fields, and nothing said so: a `Json<T>` or `Form<T>` body as an object without properties, with a `{}` example the server rejects, and a `Query<T>` with no parameters. serde reads such a struct as a map, which names no fields, so they are still not inferred. Debug builds now name each such input at startup, with how to describe it by hand; the example is dropped, and a type containing such a struct, such as a `Vec` of them, is described as any value rather than as that struct's object. Real maps are not reported, including maps keyed by a `#[serde(field_identifier)]` enum. A struct with its own `#[serde(expecting = "..")]` is taken for a map, as before. (#220)
* Routes naming one position differently - `GET /users/{id}` beside `POST /users/{name}`, allowed since #226, or `GET /files/{*path}` beside `POST /files/{id}` - were described as separate paths, which OpenAPI 3.0 forbids, so validators rejected the document. Each document now describes them under one path, named as most of its routes there are written (alphabetically first on a tie), and renames the other routes' path parameters by position. The wire reads path parameters by position, but the handler's own names are no longer the ones described, so debug builds report each renamed route at startup. (#253)
* A route with a request body of its own but no example - one described by hand, or a flattened type - took the example of an enclosing group's body, of another type. A group's example now reaches a route only with the group's body. (#260)

# 0.11.1

## Added
* `RouteGroup::map_fallback`: a fallback scoped to the group's prefix, answering any method where no route is mapped under it - a route still answers `405` for a method it lacks. The router resolves it like a route, so the most specific prefix wins: `/api/v2` over `/api`, and `/api` over a `/{*path}` route or an SPA shell under `/`. The middleware and CORS policy of the group and of every enclosing group apply, so an unknown path under an `authorize`d group is refused like a known one. It takes the arguments `App::map_fallback` takes and binds only the parameters of its prefix. It is not listed or described in OpenAPI, and `HttpContext::matched_route` is `false` for it, so a CORS preflight it alone answers gets no `204`. Mapping another one at the same prefix replaces it. (#257)

## Changed
These apply on upgrade, with no API break and no opt-in:
* The fallback file answers `GET` and `HEAD` only; any other method gets `405` with `Allow: GET,HEAD`, as a route does for a method it lacks. It used to answer every method with the shell and `200`, so a mistyped API call got HTML where it expected a `404`. A client relying on the shell from a write gets the `405`. (#256)
* `map_fallback_to_file` - and `use_static_files` with a fallback file - serves the file through an implicit `GET` route at the mount point and at `{*path}` below it, instead of taking the application's fallback slot. It no longer replaces `map_fallback` or is replaced by it. Routes at a path answer ahead of it, a `GET` route mapped at one of its positions takes that position over, and it stays out of the route listing and OpenAPI. Under a root mount every path has a route, so a non-`GET` request that used to reach `map_fallback` gets `405`. (#256)

## Fixed
* A throughput regression from 0.11.0: every request cloned the server's `ShutdownHandle`, whose `CancellationToken` locks one server-wide mutex on clone and drop, so requests queued on it under load. The handle is now cloned once per connection, and the token only for a handler that extracts `ShutdownHandle`. Plaintext benchmark, requests a second: 932K before, 1017K after, 1045K for 0.10.1. (#259)
* A route mapped twice in one group under two spellings - `/{id:integer}`, then `/{id}` - ran the group's middleware twice per request, and a sub-group respelling its parent's route did the same to the parent's. The group now records a route once, under the spelling it was mapped as last. (#258)
* `RouteGroup::use_static_files` served the fallback file across the whole application, and outside the group's middleware. It is served under the group's prefix now, inside its middleware, as the group's files are. (#257)

# 0.11.0

## Added
* Synchronous request handlers. A handler with nothing to await - formatting, arithmetic, a lookup, a check of a header - can be a plain `fn` or a closure returning its response directly, instead of opening an `async` block to hand back a ready future: `app.map_get("/sum/{x}/{y}", |x: i32, y: i32| x + y)`. The shape is read off the signature, and extraction, OpenAPI description and the response are the same as for the asynchronous form. Extractors still run before the handler, so `Json<T>`, `Form<T>`, `Query<T>` and `Dc<T>` arrive with the body already read. Accepted by `map_get` ... `map_query`, `map`, their `RouteGroup` counterparts, `map_fallback`, `map_err` and `map_conn`, and by `map_msg` for a WebSocket message handler returning its reply directly: `app.map_msg("/ws", |msg: String| format!("echo: {msg}"))`. (#250)
...
* Catch-all route parameters: a route's last segment can be `{*name}`, which binds the rest of the path as one value - `app.map_get("/files/{*path}", |path: String| ...)` answers `GET /files/a/b/c.txt` with `path = "a/b/c.txt"`. It reads at least one segment, so `/files` is not answered by it and can be mapped on its own; the value is the path as the request wrote it, trailing separator included and nothing percent-decoded, as for a single-segment parameter. At every position a literal is read first, a parameter second and a catch-all last, and the first position two routes differ at decides between them, in any registration order: `/assets/{*path}` answers `/assets/app.js` ahead of `/{lang}/{page}`. A segment after a catch-all - a group prefix ending in one included - panics where the route is mapped, and one verb naming the catch-all twice panics as a parameter named twice does (#226). Catch-all routes are listed as `{*name}` and described in OpenAPI as the path parameter `{name}`; a catch-all sharing its position and method with a parameter route - `/files/{name}` beside `/files/{*path}` - would be the same templated path there, so it is left out of every document the parameter route is described in, with a warning at startup in debug builds, and stays in the documents only it is bound to. The tail is not normalized: a `..` segment reaches the handler as it was sent, and a handler joining the value onto a directory has to reject it. (#227)
* `App::with_shutdown_timeout(Duration)` and the `[server] shutdown_timeout_secs` config key set how long a shutdown waits for open connections to finish before it closes them. The default stays 10 seconds; `Duration::ZERO` closes them right away. (#254)
* `ShutdownHandle` is an extractor, available whether or not the app was given one with `App::with_shutdown`. Its `cancelled()` resolves as soon as the shutdown starts, so a response that never ends on its own can end itself rather than hold the shutdown up until the timeout: `SseStream::new(events.take_until(shutdown.cancelled()))`. The request's `CancellationToken` is not cancelled then, since requests in flight are still answered. (#254)
...
* A `Json<T>` sent over a WebSocket - the reply of a `map_msg` handler, or `WebSocket::send` / `WsSink::send` - went out as a binary frame, so a browser client reading `event.data` got a `Blob` instead of the JSON text, and a `JSON.parse` on it failed. It is sent as a text frame now, as JSON is UTF-8 text (RFC 8259 Section 8.1). A client that read those replies as binary has to read text instead. Receiving is unchanged: `Json<T>` is still parsed from a text or a binary frame. (#250)
* The shutdown timeout closed nothing: it only stopped `run` from waiting, so a connection still open then - an SSE feed, a proxied stream - kept being served after `run` returned, for as long as the runtime lived. A binary hid this by exiting; tests, an embedded app or an in-process restart did not. The connections still open when the timeout runs out are closed now, and `run` returns once they are gone, including those of the HTTPS redirect listener, which it did not wait for at all. (#254)
* `ShutdownHandle::cancelled()` returned a future that borrowed the handle, despite its `'static` bound: under the Rust 2024 capture rules it captured `&self`, so it could not outlive the handle it came from, as in `stream.take_until(handle.cancelled())` returned from a handler. It captures nothing now. (#254)
...
* Route lookup reads a path in two passes: a greedy walk that answers every request it can without backtracking, and a depth-first search, which also finds catch-alls, for the rest. Path segments are no longer re-validated as UTF-8 on the way. Measured on the lookup alone, against 0.10.1: a static or parameter route is found 5-20% faster (7 static segments 45 -> 36 ns); a request that needs backtracking - a miss, a catch-all, a parameter behind a dead-end literal - costs 1.5-1.7x as much (a deep miss 59 -> 90 ns, a shallow one 14 -> 23 ns), and a search that gives up a branch allocates nothing for it. (#227)
...
* A request's `CancellationToken` is still its connection's: cancelled when the connection fails and, now, when the shutdown timeout closes it. The handler is dropped along with the connection, so what sees that cancellation is whatever the handler handed the token to, such as a spawned task. Cancelling the token from a handler notifies its clones and closes nothing.

# 0.10.1

## Changed
These apply on upgrade, with no API break and no opt-in:
* Bearer auth: a token that fails validation is answered `401` instead of `403`, as RFC 6750 Section 3.1 assigns `invalid_token` - including a token that does not decode. A credential that is not a bearer value is answered `400`, and a validation that could not complete, such as one against an unreadable verification key, `503`. A valid token without the required role or permission still gets `403` with `insufficient_scope`. (#246)
* `App::run` validates the dependency graph and returns `Err` on a cycle or a missing registration, before it starts any background task, the HTTPS redirect listener or the greeter. Such a registration used to fail only the requests that resolved it, with `500`. (#246)
* The `ETag` of the index and fallback files is derived from their contents, so every client revalidates them once after the upgrade. Asset tags are unchanged. (#233)
* `#[http_header]` rejects a struct with fields, and `#[derive(Claims)]` rejects enums and unions. Both used to compile and ignore what they were given. (#221)
* HTTPS redirection answers `400` instead of `404` to a request without a usable host - no `Host`, more than one, or an invalid one (RFC 9112 Section 3.2). (#247)
* HSTS `exclude_hosts` match by host name on any port, and a port in the list is ignored: `example.com:8443` now excludes `example.com` on every port, as browsers apply HSTS per host. (#247)
* A transient resolved through `Container::resolve` is moved out rather than cloned, so a `Clone` or `Drop` with side effects runs once less per resolution. (#246)

## Fixed
* Graceful shutdown was skipped for every open connection when a request was being handled at the moment the accept loop stopped, and `run` returned while responses were still being written - cut off by whatever dropped the runtime next, `run_blocking` included. `run` now always waits for open connections, up to the 10 s timeout, and releases the app's services after that. A TLS handshake still in progress is dropped rather than waited for. (#217)
* With the `http2` feature enabled (`full` included), the HTTPS redirect listener accepted only HTTP/2, so browsers got no redirect. It now serves HTTP/1.1 and HTTP/2. (#247)
* HTTPS redirection and HSTS read the host from the `Host` header only, which HTTP/2 does not send (RFC 9113 Section 8.3.1): HTTP/2 requests to the redirect listener were answered `404`, and `exclude_hosts` never matched over HTTP/2. The host is now taken from the request target first and `Host` second. A `Host: [::1]` without a port no longer fails the redirect with `500`. (#247)
* HSTS `exclude_hosts` read from a config file were stored as written, so an entry with a port or a trailing dot never matched. They are normalized like those set in code. (#247)
* Route lookup committed to the first literal it matched, so `GET /a/{b}` stopped answering `/a/b` as soon as `/a/b/c` was mapped. It now backtracks to the nearest parameter it passed over. Literals still take precedence wherever they lead to a route, a literal with a handler for another method still answers `405`, and a lookup visits each node at most once. (#246)
* A dependency cycle among services deadlocked the worker thread (scoped) or overflowed the stack and aborted the process (transient). It now panics naming the cycle, `A -> B -> A`. (#246)
* Two versions of a static file with the same length and the same whole-second `mtime` got the same `ETag`, so a client holding the old one was answered `304` for changed content - the usual case for the `index.html` of a content-hashed build deployed with pinned timestamps (`SOURCE_DATE_EPOCH`, `tar -p`, `rsync -t`). (#233)
* `TestServer` returned before its port was bound, so a test's first request could be refused. (#246)

## Performance
* Creating a request's DI scope no longer copies the registration map; its cost follows the number of scoped registrations only. A scope over 50 singletons and 3 scoped services: 404 ns -> 20 ns; with nothing scoped, one atomic increment. (#246)
* `Container::resolve` clones a shared instance in place instead of taking an `Arc` to it, and `Dc<T>` and signed/private cookies borrow the container instead of cloning it. With 8 threads resolving at once: a plain-data singleton 190 ns -> 2 ns, a cookie key 270 ns -> 4 ns. (#246)
* The middleware chain no longer allocates for stages that only pass the request on, and a handler with no captured state no longer touches a reference count. Routing, chain and handler, measured in-process: no middleware 274 ns -> 215 ns, one global middleware 368 ns -> 257 ns. The CORS middleware clones the applicable policy once instead of twice. (#246)

## Added
* `ETagSource` (`Metadata` / `Content`) with `HostEnv::with_asset_etag`, `HostEnv::with_shell_etag` and matching getters. Assets default to `Metadata`, since they are served `immutable` and never revalidated - switch to `Content` if you narrow `CacheControl::ASSET` so they are; the index and fallback files default to `Content`, since they are served `no-cache`. A content tag is cached per file version, keyed by length and full-precision `mtime` plus the inode and change time on Unix or the creation time on Windows. (#233)
* `ContainerBuilder::validate`, the check `App::run` performs, reporting every cycle and missing registration at once. `Inject::dependencies` is a provided method a hand-written `Inject` overrides to declare what it resolves; the default declares nothing, and such types are checked when constructed instead. (#246)
* Compile errors in volga's own terms: `FromRequest`, `FromRequestRef`, `IntoResponse` and `GenericHandler` carry `#[diagnostic::on_unimplemented]`, naming the built-in extractors and where a custom type plugs in (`Json`, `Query`, `Form`, `Path`, `Header`) instead of a `pub(crate)` trait bound. (#221)

## Notes
* Static-file `ETag`s stay weak whatever their source: the compression middleware may re-encode a body after the tag is set, so the octet-equality a strong tag promises (RFC 9110 Section 8.8.1) cannot be guaranteed.
* CI now builds, lints and tests on Windows and macOS as well as Linux (#242), and `trybuild` tests pin the compile-time contracts above (#243).
