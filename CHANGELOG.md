...
* DPoP sender-constrained tokens (RFC 9449, `volga-oauth-client` feature `dpop`) (#213) - a bearer token is a password: whoever holds it may use it. DPoP binds one to a key the client holds instead, and every request carries a freshly signed proof that the sender possesses it, so a stolen token is worth nothing without the key. `Dpop` is that key plus the nonce state of the servers it talks to: `Dpop::generate` mints a throwaway `ES256` one (`generate_with` also does `ES384` and `EdDSA`) - the usual lifetime is one per session, not one per process - while `Dpop::from_pem` / `from_pem_file` adopt a key that outlives it, supplying the public half explicitly, since this crate signs and does not recover public keys from private ones - the two halves are checked against each other with one signature at construction, so a public key from another pair is refused there rather than making every request this key ever signs fail remotely as an `invalid_dpop_proof`. Cloning shares both the key and the nonces, so the client and the code making resource requests with its tokens stay in step. Like the feature above it needs the JWS backend, and it is off by default for the same reason; the two are otherwise independent - a `private_key_jwt` credential says who asked for a token, a DPoP proof says who holds it, and one request may carry both.
* `OAuthClient::with_dpop` puts a proof on every token request, whichever grant sends it. The algorithm is checked against the server's `dpop_signing_alg_values_supported` before the request leaves, since the answer to an unsupported one is an `invalid_dpop_proof` that names no alternative; the authorization request identifies the key in `dpop_jkt` (RFC 9449 Section 10), binding the code to it so a stolen code cannot be redeemed by anyone else; and a `use_dpop_nonce` refusal (RFC 9449 Section 8.2) is answered by repeating the request exactly once, carrying the nonce that refusal demanded. The retry turns on what this request's own proof carried rather than on what the shared nonce state happens to hold - concurrent requests to one origin can be challenged with the same nonce, and reading the decision out of shared state would abandon whichever of them learned it second. A server that keeps refusing has its error surfaced rather than being asked again; the bound is structural, so no answer can turn it into a loop. The repeat is a new request rather than a resend of the first one's bytes: `private_key_jwt` authenticates with a one-shot assertion, and a server recording the `jti` values RFC 7523 Section 3 invites it to record would answer a second sight of one with `invalid_client`. Nonces are remembered per origin *and* per namespace: a token endpoint and a protected resource issue unrelated nonce sequences (RFC 9449 Sections 8 and 9) even when one host serves both, so the round happens once for each rather than the two overwriting each other. The tokens come back with `token_type: DPoP`, and one that does not is refused: a server without DPoP support ignores the proof and answers with an ordinary bearer token, which `dpop_signing_alg_values_supported` is optional enough not to have warned about, and taking it would hand the caller and the token store an unbound credential in answer to an explicit `with_dpop`. Token exchange is held to the same rule only where it issues an access token - an `id-jag` or an ID token is presented elsewhere and comes back as `N_A`.
* The proof itself (RFC 9449 Section 4.2) carries `typ: dpop+jwt`, the public key by value in the `jwk` header - never by `kid`, unlike a client assertion - and the `htm` / `htu` / `iat` / `jti` claims, plus `ath`, the hash of the access token, on every request that presents one; `htu` drops the query and fragment. Signing is per request and on the hot path, so the JOSE header, which never varies for a key, is rendered and encoded once when the `Dpop` is built.
* Resource requests stay the caller's: this crate mints proofs and owns the nonce state, it does not become an HTTP client for them. `Dpop::authorize(&mut headers, &method, url, &tokens)` fills in the `Authorization: DPoP <token>` credential and the `DPoP` proof covering it, refusing a token this key cannot present - a bearer one (presenting it as `Bearer` would give up the binding silently) and equally one whose recorded binding names a *different* key, which the resource refuses on every request by comparing the proof against its `cnf.jkt`, with nothing in the answer to say why. A token recording no binding at all is taken at its word: one obtained outside this crate carries nothing to check against. `authorize` reports the nonce its proof carried - resolving the nonce and signing with it are one step, so the answer names what the request actually carried, which a separate lookup could not promise while other requests to the same origin are in flight. `Dpop::proof(&method, url)` builds a proof by hand, and `Dpop::accept_nonce(url, response.headers())` adopts a nonce from any response and returns the one it carried - what the server demanded of *that* request, which `Dpop::authorize_with_nonce` then puts in the retry. The retry deliberately carries the nonce it was handed rather than whatever the shared state holds by the time it is signed: concurrent requests to one origin can be challenged with different nonces, and re-reading the shared map would hand a retry its sibling's. `Dpop::thumbprint` is the `jkt` an authorization server binds the issued token to (RFC 9449 Section 6), for a client that has to name its key out of band.
...
* `TokenSet::is_dpop` - whether the issued access token is DPoP-bound (RFC 9449 Section 5) and so must not be presented as a bearer token. `token_type` is compared case-insensitively, per RFC 6749 Section 5.1.
* `TokenSet::dpop_jkt` - the thumbprint of the key a stored token is bound to, recorded by the client that obtained it. A `TokenStore` outlives a process while a generated key does not, so `OAuthClient::token` and `ClientCredentialsRequest::token` check a stored entry against the key in hand before serving it: one bound to a key this client cannot prove possession of is dead weight however unexpired it looks (its refresh token is bound the same way, so there is nothing to renew it with either), and a bearer entry cached before a key was configured would walk straight past the downgrade refusal above. Neither is treated as an error - it is a stale cache, so the entry is evicted and a token that fits is obtained instead. A bound token is refused by a client with no key whatever the build: a store is shared and outlives a deployment, so an entry written by a DPoP-enabled one can reach a binary compiled without the feature, which has no way at all to prove possession of it.
...
* `volga-oauth-client`'s internal transport carries an arbitrary `HeaderMap` on a request and hands the response back whole (status, headers, body) for the caller to judge, instead of one optional `Authorization` header and a parsed body. Nothing public changed; this is what lets a flow read a header off a *failed* response, which the DPoP nonce round needs - the nonce to retry with arrives in a `DPoP-Nonce` header on the refusal itself, and the error the status turns into cannot carry it.
* The key loading shared by `private_key_jwt` assertions and DPoP proofs - the algorithm-to-family mapping, the PEM header check that names which of the two a caller got wrong, and the DER path - lives in one place instead of being owned by whichever feature was written first. Nothing public changed; the message for a symmetric algorithm no longer names `private_key_jwt` specifically, since both objects it now covers prove possession of an asymmetric key.
