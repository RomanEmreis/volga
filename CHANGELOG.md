...
* Creating a request's dependency-injection scope no longer copies every registration where none of them needs a copy. A server builds one scope per request, before it knows whether the request resolves anything at all, so the cost is paid by every request that reaches the process - and the whole registration map was rebuilt each time: a fresh `HashMap`, an entry copied for every service, an `Arc` for the map on top. Only a scoped service has anything to keep per scope; a singleton hands every scope the one instance it already holds and a transient keeps nothing between resolutions, so for an application registering neither, each of those entries was being copied to itself. A container that has nothing registered scoped now shares its registrations with the scopes it creates, which turns the per-request cost from one proportional to the number of registered services into a single atomic increment - measured at 5.4 ns flat, against 90 ns at ten services and 363 ns at fifty. An application that does register a scoped service is unaffected and still gets a cell of its own per scope, since that is the whole of what a scope is for.
