...
## Added
* `RouteGroup::map_fallback` - a fallback of the group's own for the requests under its prefix that no route answers, whatever their method. It is `App::map_fallback` for one part of the URL space, and it is resolved by the router the way routes are: it is mapped at the prefix and at `{*rest}` below it, so the most specific prefix wins with no rule of its own - the fallback of `/api` answers `/api/nope` ahead of a `/{*path}` route or an SPA shell served under `/`, and that of `/api/v2` ahead of that of `/api`. It answers only where no route is mapped: a request for a method a route lacks is still that route's `405`, at the prefix itself as well. The group's middleware and CORS policy apply to it, and those of every group around it, so an unknown path under an `authorize`d group is refused the way a known one is instead of telling the caller which paths exist. It takes the arguments `App::map_fallback` takes, the parameters of a parameterized prefix included; it is not listed with the routes nor described in OpenAPI, and `HttpContext::matched_route` reads `false` for a request it answers, so a CORS preflight for a path only it answers is not answered `204`. Mapping a second one at a prefix replaces the first. (#257)
