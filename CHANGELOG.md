...
* **Breaking:** a filter's `Err` is converted through `IntoError`, as a handler's is, instead of being boxed as a `Box<dyn std::error::Error + Send + Sync>`. `E` must be `Error` or implement `IntoError`, and `FilterResult::with_error` takes the same types and answers as `Err` does. (#267)
  - An error type with a status of its own answers with it from a filter as well: `OAuthError` by its code (`invalid_token` -> `401`, `insufficient_scope` -> `403`), `ValidationError` by its status, a type of your own by its `into_error`. `Err(StatusCode)`, `Err((StatusCode, E))` and `Err(Problem)` now compile in a filter. Each of these used to answer `400` unless converted with `Error::from` first.
  - A string or a `Box<dyn std::error::Error + Send + Sync>` still answers `400` from a filter, where a handler answers `500`: it carries no status of its own, and a filter's error is taken for the reason the request is refused. A boxed `Error` or `std::io::Error` keeps its status.
  - A type implementing `std::error::Error` but not `IntoError`, such as `ParseIntError`, or `anyhow::Error` until #264, no longer compiles as a filter's `Err`. Return it as `(StatusCode::BAD_REQUEST, err)`, or implement `IntoError` for a type of your own. The compile error names what a filter can return, instead of only saying that the closure does not implement `Filter`.

## Fixed
* A filter answered `400` for every `Err`. A `volga::Error` was wrapped in a new `400` error, so the status it was created with, its instance and the response attached with `Error::with_response` were lost, and `map_err` got the wrapper with the error nested inside. A filter returning `Err(Error)`, or `FilterResult::with_error` given one, now answers with that error as it is, as a handler does: `Err(Error::from_parts(StatusCode::UNAUTHORIZED, ..))` answers `401`. An `Err(std::io::Error)` answers with the status its kind maps to, as from a handler (`NotFound` -> `404`, `PermissionDenied` -> `403`, `Other` -> `500`), so a filter's `Err` is no longer always a `4xx`. `false` and a string still answer `400`; for the other error types, see Changed. (#263)
