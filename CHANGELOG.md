...
* `ShutdownHandle` is an extractor. A handler or middleware that takes one gets the running server's handle, including in an app that was not given one with `App::with_shutdown`. Its `cancelled()` future resolves as soon as the shutdown starts, so a response that never ends on its own can end itself instead of holding the shutdown up until the timeout: `SseStream::new(events.take_until(shutdown.cancelled()))`. This is a different signal from the request's `CancellationToken`, which the start of a shutdown leaves alone, because requests in flight are still answered. (#254)
...
* The shutdown timeout did not close anything. It only stopped `run` from waiting, and a connection still open then, such as one serving an SSE feed or a proxied stream, kept serving after `run` had returned, for as long as the runtime lived. That was hidden in a binary, where the process exits, but not in tests, an app embedded in a larger process, or an in-process restart. When the timeout runs out now, every open connection is closed and its requests' `CancellationToken` cancelled, and `run` returns only once they are gone. The HTTPS redirect listener does the same. (#254)
* `ShutdownHandle::cancelled()` returned a future that borrowed the handle, despite its `'static` bound: under the Rust 2024 capture rules it captured `&self`, so it could not outlive the handle it came from, as in `stream.take_until(handle.cancelled())` returned from a handler. It captures nothing now.
...
* A request's `CancellationToken` belongs to its connection, as before: it is cancelled when the connection fails, and now also when the shutdown timeout closes the connection. A handler future is dropped when its connection is closed, so the cancellation reaches what a handler handed the token to, such as a spawned task, rather than the handler itself.
