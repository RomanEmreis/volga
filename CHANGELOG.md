...
* A route mapped twice in the same group takes that group's configuration once. The second `map_get` for a pattern lands on the endpoint the first one made, so it is one route; the group used to count it as two and prepend its middleware twice - a `token_bucket` spending two permits per request - and repeat its OpenAPI tag on the operation. Two spellings of one dynamic route (`/users/{id}` and `/users/{name}`) still count as two, since they are the ambiguity problem rather than this one.
