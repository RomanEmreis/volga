...
* The derived rules are published as well as enforced: `Validate::constraints()` reports what the fields declare, `Valid<E>` forwards it into the OpenAPI description, and it lands as `minLength` / `maxLength` / `minimum` / `maximum` on the property of the request schema or on the query parameter of the same name. A documented constraint and an enforced one therefore cannot drift apart. The method has a default returning nothing, so a hand-written `Validate` is unaffected.
