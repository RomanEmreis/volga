...
* A `HEAD` request no longer walks past the middleware guarding the `GET` it answers. `App::map_get` registers an implicit `HEAD` endpoint for the same pattern, and that endpoint was registered with the handler and nothing else: the middleware bound to the route, the middleware of the group the route belongs to and the route's CORS policy all went to the `GET` alone. So a route behind a group's `authorize` answered `403` to `GET /admin/report` and `200` to `HEAD /admin/report` - status and headers for a request the middleware existed to reject, and rate limiting, filters and `tap_req` skipped along with it. Whatever is bound to a `GET` route is now bound to its implicit twin as well. A `HEAD` mapped by hand replaces the twin rather than stacking on it, so it stays a route of its own whichever side of the `GET` it is written on, and the twin still carries no OpenAPI operation, since it is not part of the surface the application describes.
