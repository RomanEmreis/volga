...
* `OpenApiRegistry::describe_route_as`, which describes a route under the path another pattern at its position spells, renaming its path parameters by position, and `OpenApiRegistry::remove_position`, which removes every operation at one position whatever its parameters are named there - what volga describes routes naming one position differently with. (#253)

## Fixed
* A type with a `#[serde(flatten)]` field was described with none of its fields, the ones declared beside the flattened member included, and nothing said so: a `Json<T>` or `Form<T>` body was published as an object without properties, together with an example of `{}` that the server refuses, and a `Query<T>` published no parameters at all. serde reads such a struct as a map, to collect the keys it does not name for the flattened member, and a map does not say which keys it takes - so no fields can be inferred from it. That is still so, but it is no longer silent: debug builds name each such input at startup, with the type serde reads as a map and how to describe the input by hand, and the example is gone. A type holding such a struct - a `Vec` of them, or a struct with one as a field - used to be published as that struct's object, even where it is an array; its body is now described as any value. The probe tells such a struct from an actual map by the two things serde's derive gives it - a visitor expecting `struct Name`, and keys read as field identifiers - so a map body is still an object and is not reported, a map keyed by a `#[serde(field_identifier)]` enum included. A struct that renames what it expects with `#[serde(expecting = "..")]` is taken for a map, and described as before. `NamedPath<T>` is not reported either: the route template names every path parameter regardless, as it does for `Path<T>`. (#220)
* Routes naming one position differently - `GET /users/{id}` beside `POST /users/{name}`, which another verb may do since #226, or a catch-all beside a parameter route of another verb, `GET /files/{*path}` beside `POST /files/{id}` - were described as two paths, `/users/{id}` and `/users/{name}`. OpenAPI 3.0 forbids templated paths that differ in their parameter names alone ("MUST NOT exist as they are identical"), so a validator rejected the document and a code generator kept one of the two. A document now describes every route at one position under one path, named the way most of the routes it describes there are written, the first in alphabetical order on a tie: `/users/{id}`, with the `POST` operation's parameter renamed `id`. The names are chosen in each document from the routes described in it, whatever order they were mapped in, and chosen again when a route is bound to another document. A path parameter is read by position, so the wire does not notice the rename, but it is not the name the handler reads - a `NamedPath<T>` field among them - so debug builds name each route described under other names at startup, with the names it is described under. A catch-all is still left out of the documents a parameter route of its own verb is described in (#227), and names nothing there. (#253)
* A route with a request body of its own inside a group whose OpenAPI configuration describes a body - a body described by hand with `with_request_schema`, or one whose example is left out as above - took the group's example whenever it had none of its own, and was published with an example of another type than its schema. A group's example now reaches a route only together with the group's body, where the route has no body of its own. (#260)

# 0.11.1

## Added
* `RouteGroup::map_fallback` - a fallback of the group's own for the requests under its prefix that no route answers, whatever their method. It is `App::map_fallback` for one part of the URL space, and it is resolved by the router the way routes are: it is mapped at the prefix and at a catch-all below it - or at the prefix alone, when the prefix ends in a catch-all of its own - so the most specific prefix wins with no rule of its own - the fallback of `/api` answers `/api/nope` ahead of a `/{*path}` route or an SPA shell served under `/`, and that of `/api/v2` ahead of that of `/api`. It answers only where no route is mapped: a request for a method a route lacks is still that route's `405`, at the prefix itself as well. The group's middleware and CORS policy apply to it, and those of every group around it, so an unknown path under an `authorize`d group is refused the way a known one is instead of telling the caller which paths exist. It takes the arguments `App::map_fallback` takes, and binds the parameters its prefix declares and nothing else, the same at the prefix and below it; it is not listed with the routes nor described in OpenAPI, and `HttpContext::matched_route` reads `false` for a request it answers, so a CORS preflight for a path only it answers is not answered `204`. Mapping a second one at a prefix replaces the first. (#257)

## Changed
These apply on upgrade, with no API break and no opt-in:
* The fallback file answers `GET` and `HEAD` alone (#256). It used to answer every method with the shell and `200`, so a mistyped API call or one to a renamed endpoint got an HTML page where it expected a `404`, and failed far from its cause - `Unexpected token '<'` out of `res.json()`; a `POST` to the path of a file got the shell as well, and an `OPTIONS` got an HTML body. The shell is how a client-side URL renders the application, and a browser navigates with `GET`, so any other method is now answered `405` with `Allow: GET,HEAD`, as a route answers a method it lacks - whether a file, a route or nothing at all is at that path. A client that relied on getting the shell back from a write gets the `405`.
* `map_fallback_to_file` - and `use_static_files` with a fallback file - serves the file through a `GET` route at the mount point and at `{*path}` below it, rather than by taking the application's fallback slot. The two used to replace each other, whichever was registered last; `map_fallback` is now left alone, and still answers whatever lies outside the mount. The route is resolved like any other: a literal or a parameter route reading a path answers it ahead of the shell, and a `GET` route mapped by hand at one of the shell's own positions - `app.map_get("/{*rest}", ..)` beside a root mount - takes that position over in either registration order, instead of panicking as a second name for a route would. It is left out of the route listing and of the OpenAPI document, as the files are. Under a root mount serving the file, every path has a route, so a non-`GET` request that used to reach `map_fallback` is answered `405` there now.

## Fixed
* Every request cloned the server's `ShutdownHandle` into its scope, and a `ShutdownHandle` is a `CancellationToken`, whose clone and drop lock a mutex that every clone of that token shares. Since 0.11.0, then, every request locked one mutex for the whole server twice, on every worker, and under load the requests queued on it: on the plaintext benchmark (10 server CPUs, 1024 connections) 0.11.0 served about 8% fewer requests a second than 0.10.1. The handle is now cloned once per connection and shared by that connection's requests, and the token itself is cloned only for a handler that extracts `ShutdownHandle`. Measured in one session, with the order rotated: 932K requests a second before the fix, 1017K after it, and 1045K for 0.10.1. (#255)
* A route mapped twice in one group under two spellings of its parameters - `/{id:integer}`, then `/{id}` - is one route, the second replacing the first, but the group recorded both spellings and ran its middleware twice for every request to it. A sub-group respelling a route its parent mapped did the same to the parent's middleware. The group now records a route once, by the position the router reads, under the spelling it was mapped as last - the one the OpenAPI document keeps.
* `RouteGroup::use_static_files` served the fallback file across the whole application rather than under the group's prefix: with a mount at `/static`, `GET /api/nope` and `GET /totally/elsewhere` were answered with the shell. It is served under the group's prefix now, as the group's files are, and the group's middleware runs around it - it used to run around the files but not the shell. (#257)
