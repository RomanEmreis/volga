...
## Added
* `volga::error::IntoError`, which turns the `Err` of a handler's `Result<T, E>` into an `Error` for the error handler. It is implemented for every type with `From<T> for Error` (`Error` itself, `std::io::Error`, `serde_json::Error`, `Infallible` and the rest), for `StatusCode` (that status, with its canonical reason as the message), for `(StatusCode, E)` where `E` is a message or any error, for `String`, `&'static str`, `Cow<'static, str>` and `Box<str>` (`500` with that message), for `Box<dyn std::error::Error + Send + Sync>` (`500`), and for `Problem<E>`. For a type of your own, implement `IntoError`, or `From<T> for Error`, which gives you `IntoError` as well. There are deliberately no impls for integers: `Err(404)` could mean a status or an application's error code, and `Err(StatusCode::NOT_FOUND)` is unambiguous. An `Err` type with neither impl is a compile error that names `IntoError`. (#262)
* `Error::with_response`, `Error::has_response` and `Error::take_response` let an error carry the response it answers with, such as a JSON envelope or a `Problem`, while still going through the error handler. `map_err` receives the error and can either answer with something else or return the error to use the attached response. The default handler and `use_problem_details` answer with the attached response unchanged. The response is sent with the error's own status, so a bare body such as `Json(..)` can be attached. `Error` grows by one pointer, and `HttpResult` stays the same size as `HttpResponse`. (#262)
* `IntoError::describe_openapi` lets an error type describe its own responses in a route's OpenAPI operation. `Result<T, E>` now describes both `T` and `E`. The default describes nothing, since the error handler decides what an error looks like. (#262)
...
* `tap_req` still takes `Result<HttpRequestMut, Error>` only. Because it has a single `Result` impl, rustc can infer the error type of a `?` followed by a bare `Ok(..)`. Allowing any `IntoError` here would break that inference. An error of another type reaches it through `?` when it has `From<T> for Error`, or as `Err(err.into_error())`. (#262)
