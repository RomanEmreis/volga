...
## Added
* `#[derive(Validate)]` (feature `validation-derive`, #218) - writes out the same `validate()` a hand-written impl would, from the bounds declared on the fields. It is not magic and it is not a second way to validate: the trait stays the seam, so a derived impl and a hand-written one are indistinguishable to `Valid<E>`. Field rules are `length(min, max, equal)` for strings and collections, `range(min, max)` for anything ordered, `nested` for a field validating itself, and `custom = "path::to::fn"`; each takes an optional `message = ".."` that replaces the default text. A rule spanning two fields cannot live on either of them, so the container takes `schema = "path::to::fn"` for those - without it, cross-field rules would have nowhere to go, which is the reason the derive is sugar over the trait rather than a replacement for it. All failures are collected rather than stopping at the first, and every bound and message is rendered at expansion time, so a failing check pushes a `&'static str` and allocates nothing. A status asked for by a merged failure is kept, so `ValidationError::with_status` works from a `schema` or `custom` function exactly as it does from a hand-written impl.
