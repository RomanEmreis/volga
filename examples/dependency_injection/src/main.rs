...
async fn main() -> std::io::Result<()> {
    ...
    let global_cache = InMemoryCache {
        inner: Default::default(),
    };
    app.add_singleton(global_cache) // Register a singleton service that is available globally
...
async fn set_value<T: Cache>(Json(item): Json<Item>, cache: Dc<T>) {
