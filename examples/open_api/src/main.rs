...
use volga::{
    App, File, Form, Json, Multipart, NamedPath, Path, Query, http::sse::Message, ok, sse_stream,
};

#[tokio::main]
async fn main() -> std::io::Result<()> {
    let mut app = App::new().with_open_api(|config| {
        config
    ...
    app.group("/path", |api| {
        ...
        api.map_get(
            "/{name}/{age:integer}",
            async |Path((_name, _age)): Path<(String, u32)>| {},
        );

        api.group("/named", |api| {
            api.map_get("/{name}/{age}", async |path: NamedPath<Payload>| {
                ok!(path.into_inner())
            })
            .open_api(|cfg| cfg.produces_json::<Payload>(200).produces_no_schema(400));
        });
    });
    ...
    app.group("/files", |api| {
        ...
        api.map_post("/multipart", async |mut files: Multipart| {
            sse_stream! {
    ...
    app.map_get(
        "/query",
        async |q: Query<Payload>| ok!(fmt: "Hello {}", q.name),
    )
    .open_api(|cfg| cfg.with_docs(["v1", "v2"]));

    app.map_put("/form", async |payload: Form<Payload>| payload)
        .open_api(|cfg| cfg.with_doc("v2").produces_form::<Payload>(200u16));

    app.map_post("/json", async |payload: Json<Payload>| payload)
        .open_api(|cfg| {
            cfg.with_doc("v2").produces_json_example(
                200u16,
                Payload {
                    name: "John".into(),
                    age: 30,
